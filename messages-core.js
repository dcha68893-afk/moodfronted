// =============================================
// MESSAGES-CORE.js - HARDENED PRODUCTION CORE ENGINE v3.0.4
// SECURE PARENT-IFRAME COMMUNICATION LAYER
// FIXED: Handshake protocol, signature validation, parent communication
// =============================================
...
(function() {
    ...
    const SecurityUtils = {
        ...
        verifySignature(message) {
            if (!message || !message.signature || !message.timestamp) return true;
            ...
            if (!isValid) {
                Logger.debug('Security', `Invalid signature for ${message.type}`);
            }
    ...
    const SessionMirror = {
            ...
            this._initPromise = new Promise((resolve) => {
                try {
                    ...
                    if (cached && cached.expiresAt > Date.now()) {
                        ...
                        Logger.info('Session', 'Session restored from cache');
                    }
                } catch (e) {
        ...
        _setupRefreshTimer() {
            ...
            if (timeUntilExpiry > 0 && timeUntilExpiry < 300000) {
                ...
                }, Math.max(timeUntilExpiry - 60000, 1000));
            }
        },
            ...
            if (window.parent && window.parent !== window) {
                try {
                    window.parent.postMessage({
                        type: MESSAGE_TYPES.SESSION_REFRESH,
            ...
            return this.circuitBreaker.call(() => {
                ...
                if (data.type !== MESSAGE_TYPES.HANDSHAKE_REQUEST && 
                    ...
                    if (!SecurityUtils.verifySignature(data)) {
                        Logger.warn('Firewall', 'Invalid signature for ' + data.type);
                        return false;
                    }
                }
    ...
    const HandshakeClient = {
        ...
        _performHandshake() {
            ...
            const handshakeMessage = MessageFirewall.createOutbound(
                ...
                {
                    version: VERSION,
                    compatibleVersions: ['2.0.0', '2.0.4', '3.0.0', '3.0.1', '3.0.2', '3.0.3', '3.0.4'],
            ...
            try {
                ...
                Logger.debug('Handshake', 'Handshake request sent', handshakeMessage.id);
            } catch (e) {
                this._completeLegacy();
            }
        },
        ...
        _completeLegacy() {
            Logger.info('Handshake', 'Completing with legacy mode');
        ...
        complete(response) {
            ...
            if (!this._isVersionCompatible(payload.version)) {
                Logger.warn('Handshake', `Incompatible version: ${payload.version}, using legacy`);
                this._completeLegacy();
                return false;
            }
        ...
        _isVersionCompatible(version) {
            ...
            const compatible = ['2.0.0', '2.0.4', '3.0.0', '3.0.1', '3.0.2', '3.0.3', '3.0.4'];
    ...
    class SecureMessagingClient {
        ...
        async _receive(event) {
            await this.circuitBreaker.call(async () => {
                ...
                if (message.type === MESSAGE_TYPES.HANDSHAKE_RESPONSE) {
                    HandshakeClient.complete(message);
                }

                if (message.type === MESSAGE_TYPES.SESSION_DATA || 
                    message.type === MESSAGE_TYPES.SESSION_INIT ||
                    ...
                    if (event.source) {
                        try {
                            event.source.postMessage({
                                id: `ack_${message.id}`,
                                type: MESSAGE_TYPES.SESSION_ACK,
                                payload: { 
                ...
                if (message.type === MESSAGE_TYPES.PARENT_READY) {
                    ...
                    HandshakeClient.complete({
                        type: MESSAGE_TYPES.HANDSHAKE_RESPONSE,
                        payload: { version: '3.0.4', serverTime: Date.now() }
                    });
                }
                ...
                if (message.type === MESSAGE_TYPES.FORCE_RELOAD) {
        ...
        async send(type, payload = {}, options = {}) {
            return this.circuitBreaker.call(async () => {
                ...
                if (options.requiresAck !== false) {
                    return new Promise((resolve) => {
                        ...
                        try {
                            ...
                            Logger.debug('Messaging', `Sent: ${type} (waiting ACK)`, message.id);
                        } catch (error) {
                ...
                try {
                    window.parent.postMessage(message, '*');
                    Logger.debug('Messaging', `Sent: ${type} (fire & forget)`, message.id);
                    return { success: true };
                } catch (error) {
                    Logger.error('Messaging', `PostMessage failed: ${error.message}`);
        ...
        async processQueue() {
            if (this.messageQueue.length === 0 || !ParentDetector.isReady) return;

            const now = Date.now();
            const freshQueue = this.messageQueue.filter(msg => msg.timestamp > now - 3600000);
    ...
    ParentDetector.subscribe((ready) => {
        isParentReady = ready;
        window.dispatchEvent(new CustomEvent('parentStatusChanged', { 
            detail: { ready } 
        }));
    });
    ...
    async function runStage(stage, fn, timeoutMs = 5000) {
        ...
        try {
            ...
            if (stage === INIT_STAGES.HANDSHAKE) {
                Logger.warn('Init', 'Handshake failed, continuing with legacy mode');
        ...
        try {
            await runStage(INIT_STAGES.PREFLIGHT, async () => {
                ...
                if (window.parent && window.parent !== window) {
                    try {
                        ...
                        Logger.debug('Init', 'CHILD_READY sent');
                    } catch (e) {}
                }
                ...
                if (!ParentDetector.isReady) {
                    ...
                    await new Promise(resolve => {
                        ...
                        const checkInterval = setInterval(() => {
                            attempts++;
                            if (ParentDetector.isReady || attempts > 20) {
            ...
            await runStage(INIT_STAGES.SESSION, async () => {
                ...
                const sessionResult = await messagingClient.send(
                    ...
                    { requiresAck: false, timeout: 5000 }
                );

                return { success: sessionResult?.success || false };
            }, 10000);
...
${message.fileSize ? `Size: ${formatFileSize(message.fileSize)}\n` : ''}`;
    ...
    Logger.info('Core', `Messages Core v${VERSION} loaded`);
})();