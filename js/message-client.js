...
(function () {
    ...
    async function _directRequest(method, path, body) {
        // Critical reads and sends use the runtime-config transport directly.
        // This avoids iframe bootstrap races while keeping every other request
        // on the existing API_REQUEST/API_RESPONSE contract.
        if ((method === 'GET' && /^\/chats(?:\?|$)/.test(path) ||
            ...
            try {
                ...
                const token = window.__kynToken || window.__accessToken ||
                    window.AuthSessionManager?.getToken?.() || window.authToken ||
                ...
                if (error?.name === 'AbortError') {
                    throw new Error(
                        method === 'POST'
                            ...
                            : 'Conversation list request timed out'
                    );
                }
                throw error;
            } finally {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Keep this watchdog above chat.html/api.core.js's 45s network
            // ceiling so a slow Render cold start cannot turn a request into
            // a false failure before the real API_RESPONSE arrives.
            ...
            const handler = (event) => {
                ...
                // Defend against either {success,data:{...}} or an already
                // unwrapped response without adding another transport layer.
            ...
            window.parent.postMessage({
                type: 'API_REQUEST',
                payload: { endpoint: path, method, body, requestId },
            }, '*');
        });
    }
