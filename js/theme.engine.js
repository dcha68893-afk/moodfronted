...
(function (global) {
  ...
  function atomicPaint(reason){beginBoot(document);paintNow(state.theme,state.fontSize,state.accentColor,state.iconScale,document);endBoot(document);if(reason)notify(reason);}
  ...
  var ThemeManager={__kynEngine:true,getTheme:function(){return state.theme;},getFontSize:function(){return state.fontSize;},getAccentColor:function(){return state.accentColor;},getIconScale:function(){return state.iconScale;},setTheme:function(v,opts){var n=validateTheme(v);if(n===state.theme&&!(opts&&opts.force))return n;state.theme=n;set(THEME_KEY,n);atomicPaint('theme');return n;},setFontSize:function(v){var n=validateFont(v);if(n===state.fontSize)return n;state.fontSize=n;set(FONT_KEY,String(n));atomicPaint('fontSize');return n;},setIconScale:function(v){var n=validateIcon(v);if(n===state.iconScale)return n;state.iconScale=n;set(ICON_KEY,n);atomicPaint('iconScale');return n;},setAccentColor:function(v){if(!v||v===state.accentColor)return state.accentColor;state.accentColor=v;atomicPaint('accentColor');return v;},onChange:function(fn){if(typeof fn!=='function')return function(){};listeners.push(fn);return function(){var i=listeners.indexOf(fn);if(i>=0)listeners.splice(i,1);};},broadcastToIframe:function(frame,page){injectFrame(frame,page||frame.dataset&&frame.dataset.module);},broadcastToAllIframes:function(selector){try{document.querySelectorAll(selector||'iframe.content-iframe,iframe').forEach(function(f){injectFrame(f,f.dataset&&f.dataset.module);});}catch(_) {}}};
