...
(function (global) {
  ...
  var PALETTES = {
    dark: {
      /* ROOT-CAUSE FIX (ICONS/TEXT INVISIBLE + FLICKER PER-THEME ACROSS
         MODULES): Tools.html/status.html/calls.html/friend.html reference
         --danger-color/--error-color/--success-color/--warning-color/
         --info-color/--hover-bg/--overlay-bg/--shadow/--shadow-lg/
         --transition, none of which this palette ever defined — so every
         rule using them silently kept its hardcoded literal fallback (often
         a light-mode color) in BOTH themes, which is exactly why some
         icons/text stayed the wrong, sometimes invisible, color regardless
         of which theme was active. Aliased to the real theme colors instead
         of inventing new hardcoded values. */
      '--danger-color':'#ef4444','--error-color':'#ef4444','--success-color':'#22c55e','--warning-color':'#f59e0b','--info-color':'#38bdf8','--hover-bg':'rgba(255,255,255,.06)','--overlay-bg':'rgba(0,0,0,.65)','--shadow':'0 2px 8px rgba(0,0,0,.4)','--shadow-lg':'0 16px 48px rgba(0,0,0,.6)','--transition':'all .2s ease',
    ...
    light: {
      '--danger-color':'#ef4444','--error-color':'#ef4444','--success-color':'#22c55e','--warning-color':'#f59e0b','--info-color':'#38bdf8','--hover-bg':'rgba(0,0,0,.04)','--overlay-bg':'rgba(0,0,0,.45)','--shadow':'0 2px 8px rgba(0,0,0,.08)','--shadow-lg':'0 16px 48px rgba(0,0,0,.16)','--transition':'all .2s ease',
