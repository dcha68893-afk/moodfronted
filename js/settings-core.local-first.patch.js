 ...
 *   ✅  Coordinates with NecpaSettingsManager after local load
...
if (window.__SETTINGS_LOCAL_FIRST_PATCH__) {
    ...
    (function applyLocalFirstPatch() {
        ...
        function _syncToNecpaManager(settings) {
            const mgr = window.NecpaSettingsManager;
        ...
        window.addEventListener('settingsLoaded', (e) => {
            if (e.detail && e.detail.settings && !e.detail.fromLocal) {
                ...
                _syncToNecpaManager(e.detail.settings);
            }
        });
