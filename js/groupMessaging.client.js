...
/* FIX (SubtleCrypto.sign "parameter 2 is not of type CryptoKey"): a RECEIVER copy of your own key (built from the
   server's distribution when the local copy is missing or when reading your own history) has no private signing key.
   It used to share one storage slot with the real SENDER state, so it overwrote it (or was picked up as it) and every
   later send failed to sign. Receiver copies of your own key now live in a separate `_rx` slot, and receiver copies are
   additionally tagged with the sender's public-key fingerprint so a sender that re-keys never breaks older messages. */
const idkey=(g,e,o,rx,fp)=>`kyn_gsk_v2_${g}_${e}_${o}${rx?'_rx':''}${fp?'#'+fp:''}`;
const fpOf=pub=>{try{return String(pub&&pub.x||'').slice(0,12)}catch(_){return ''}};
...
async function distribute(g,e,st,members){const live=withSelf(await liveMembers(g,members));if(!live.length)throw new Error('No current group members are available for sender-key distribution');const raw=b64(te.encode(JSON.stringify({chain:b64(st.chain),iteration:st.iteration,publicJwk:st.publicJwk}))),ds=[],failed=[];for(const id of live){try{ds.push({userId:id,deviceId:'primary',ciphertext:await wrapKey(raw,id),algorithm:ALGORITHM})}catch(error){failed.push({userId:id,message:error?.message||'Recipient key unavailable'})}}if(!ds.some(d=>Number(d.userId)===Number(st.ownerId))){const sf=failed.find(f=>Number(f.userId)===Number(st.ownerId));throw new Error('Your own group sender key could not be prepared'+(sf?': '+sf.message:(st.ownerId>0?'':': your user id could not be determined')))}await req('/group-messages/'+encodeURIComponent(g)+'/crypto/rotate',{method:'POST',body:JSON.stringify({epoch:e,distributions:ds})});states.set(idkey(g,e,st.ownerId),st);await saveState(st);return st}
async function ensureSenderKey(g,members){if(!(await waitReady()))throw new Error('Secure group identity is not ready');const gid=Number(g),owner=me(),live=withSelf(await liveMembers(gid,members));if(!owner)throw new Error('Your account id is not available yet; retry in a moment');if(!live.length)throw new Error('No current group members are available for sender-key distribution');for(let attempt=0;attempt<4;attempt++){const s=await state(gid,true),epoch=Math.max(1,Number(s.epoch)||1),entry=keyEntry(s,epoch,owner);let st=states.get(idkey(gid,epoch,owner))||await loadState(gid,epoch,owner);const missing=Array.isArray(s?.missingMemberIds)?s.missingMemberIds.map(Number).filter(Boolean):[];if(st&&!st.privateKey)st=null;/* FIX: no signing key locally (new device / cleared storage / old corrupted state) -> create a fresh sender key and redistribute it instead of trying to sign with nothing */if(st&&entry?.distribution&&!missing.includes(owner)){states.set(idkey(gid,epoch,owner),st);return st}if(st){try{return await distribute(gid,epoch,st,live)}catch(e){if(e.code==='GROUP_SENDER_KEY_INCOMPLETE'||e.code==='GROUP_SENDER_KEY_EPOCH_MISMATCH'||e.status===409){await sleep(500);continue}throw e}}st=await createSenderState(gid,epoch);try{return await distribute(gid,epoch,st,live)}catch(e){if(e.code==='GROUP_SENDER_KEY_INCOMPLETE'||e.code==='GROUP_SENDER_KEY_EPOCH_MISMATCH'||e.status===409){await sleep(500);continue}throw e}}throw new Error('Group sender key is still synchronizing; retry in a moment')}
async function getReceiverState(g,e,o,fp){const rx=Number(o)===me(),k=idkey(g,e,o,rx,fp);let st=states.get(k)||await loadState(g,e,o,rx,fp);if(st){states.set(k,st);return st}
/* legacy cache entry (saved before fingerprint tagging): accept it only if it is the same sender key the message was signed with */
if(fp){const k0=idkey(g,e,o,rx),s0=states.get(k0)||await loadState(g,e,o,rx);if(s0&&fpOf(s0.publicJwk)===fp){states.set(k0,s0);return s0}}
const s=await state(g,true),entry=keyEntry(s,e,o);if(!entry?.distribution)throw new Error('Sender key for this group message is not available');const raw=await unwrapKey(entry.distribution.ciphertext,o),b=JSON.parse(td.decode(unb64(raw)));
/* the sender re-keyed since this message was written and the old key is no longer on the server */
if(fp&&fpOf(b.publicJwk)!==fp)throw new Error('Sender key for this group message is not available');
st={slot:k,rx,groupId:Number(g),epoch:Number(e),ownerId:Number(o),chain:unb64(b.chain),privateKey:null,privateJwk:null,publicJwk:b.publicJwk||null,iteration:Number(b.iteration)||0/* FIX: a re-distributed, already-advanced chain must start counting at its real position */,skipped:new Map()};states.set(k,st);await saveState(st);try{await req('/group-messages/'+encodeURIComponent(g)+'/crypto/ack',{method:'POST',body:JSON.stringify({ownerId:o,epoch:e})})}catch(_){/* key is installed locally; acknowledgement can retry during next sync */}return st}
...
/* FIX (RACE THAT CORRUPTED THE RATCHET / MESSAGES STUCK ON "Decrypting..."): a sender-key chain is one-way and its state object
   ...
   state at the same time — wrong message keys, AES-GCM failures, and a chain that had already moved past messages that were then
   "too old" to read. All decrypts for one group now run strictly one after another, in the order they were requested. */
const decryptChains=new Map();
function decryptForGroup(g,ciphertext,members){
  const k=String(g),prev=decryptChains.get(k)||Promise.resolve();
  const run=prev.then(()=>decryptForGroupUnsafe(g,ciphertext,members),()=>decryptForGroupUnsafe(g,ciphertext,members));
  const tail=run.then(()=>{},()=>{});
  decryptChains.set(k,tail);
  tail.then(()=>{if(decryptChains.get(k)===tail)decryptChains.delete(k)});
  return run;
}
