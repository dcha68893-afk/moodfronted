  ...
  class UnifiedAppCache {
    ...
    async getAll(collection) {
      ...
      if (storeName === 'chats' || storeName === 'groups') {
        const tombstones = (() => {
          try { return JSON.parse(localStorage.getItem('necpa_tombstones_v1') || '{}'); } catch (_) { return {}; }
        })();
    ...
    async deleteConversation(chatId) {
      ...
      // 1. Write tombstone
      try {
        const tombstones = JSON.parse(localStorage.getItem('necpa_tombstones_v1') || '{}');
        tombstones[id] = { deletedAt: nowTs(), entityType: 'chat', syncRevision: 1 };
        localStorage.setItem('necpa_tombstones_v1', JSON.stringify(tombstones));
      } catch (_) {}
      // 2. Remove from IDB
    ...
    setSession(session) {
      ...
      try {
        ...
        ["authToken", "token", "accessToken", "necpa_token", "USER_TOKEN"].forEach((key) => {
    ...
    clearSession() {
      try {
        ...
        ["authToken", "token", "accessToken", "necpa_token", "USER_TOKEN", "currentUser", "user", "necpa_user", "isLoggedIn"].forEach((key) => localStorage.removeItem(key));
        return true;
      } catch (_error) {
        return false;
      }
