...
function parentNavigationBridge(){
  window.addEventListener('message',function(ev){
    const d=ev.data||{};
    if(d.type!=='GO_BACK_TO_LIST'&&d.type!=='GROUP_BACK_TO_LIST')return;
    // The group iframe owns both the sidebar and the chat body. Returning only
    // the parent header leaves the iframe stuck on the selected group. Always
    // restore the iframe's own list state first, then notify the shell.
    try{
      const layout=document.getElementById('layout');
      if(layout)layout.classList.remove('chat-open');
      const messages=document.getElementById('messages');
      if(messages)messages.innerHTML='<div class="empty">Select a group to start chatting.</div>';
