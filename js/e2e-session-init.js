 ...
 * Session state is keyed by user pair, never by conversation/chat id.
 */
(function () {
  'use strict';
  ...
  async function publishBundle(b){
    const one=(b.oneTimePreKeys||[]).map(k=>({keyId:k.keyId,publicKey:k.publicKey}));
    const r=await request('/api/encryption/prekeys',{method:'POST',body:JSON.stringify({signingPubKey:b.signingPublicKey,signedPreKey:{keyId:b.signedPreKey.keyId,publicKey:b.signedPreKey.publicKey,signature:b.signedPreKey.signature},oneTimePreKeys:one})});
    if(!r.ok)throw Error(`prekey registration failed: HTTP ${r.status}`);return r.json()
  }
  async function replenish(b,count){for(let i=0,n=Math.max(PREKEY_TARGET-count,0);i<n;i++){const kp=await genPrekey();b.oneTimePreKeys.push({keyId:crypto.randomUUID(),publicKey:await exportPublic(kp.publicKey),privateKey:await exportPrivate(kp.privateKey)})}return b}
  async function provisionPrekeys(){
    ...
    saveBundle({...b,lastPublishedAt:Date.now()});return true
  }
  ...
  async function hkdf(raw,info,len=32){const k=await crypto.subtle.importKey('raw',raw instanceof ArrayBuffer?raw:raw.buffer,'HKDF',false,['deriveBits']);return crypto.subtle.deriveBits({name:'HKDF',hash:'SHA-256',salt:new Uint8Array(32),info:new TextEncoder().encode(info)},k,len*8)}
  ...
  async function verifySignedPrekey(b){if(!b.signingPubKey||!b.signedPreKey?.publicKey||!b.signedPreKey?.signature)throw Error('recipient signed prekey bundle is incomplete');const k=await crypto.subtle.importKey('spki',unb64(b.signingPubKey),{name:'ECDSA',namedCurve:'P-256'},false,['verify']);if(!await crypto.subtle.verify({name:'ECDSA',hash:'SHA-256'},k,unb64(b.signedPreKey.signature),new TextEncoder().encode(b.signedPreKey.publicKey)))throw Error('recipient signed prekey signature verification failed')}
  ...
  async function x3dhInitiate(chatId,recipientId){
    const b=await fetchBundle(recipientId);await verifySignedPrekey(b);if(!b.identityPublicKey)throw Error('recipient identity public key missing from prekey bundle');
    const my=window.KynectaE2E.getMyIdentityPrivateKey();if(!my)throw Error('local identity key is not ready');
    const ikb=await importPublic(b.identityPublicKey),spkb=await importPublic(b.signedPreKey.publicKey),eph=await genPrekey();
    const ekaPub=await exportPublic(eph.publicKey),parts=[await dh(my,spkb),await dh(eph.privateKey,ikb),await dh(eph.privateKey,spkb)];
    if(b.oneTimePreKey?.publicKey)parts.push(await dh(eph.privateKey,await importPublic(b.oneTimePreKey.publicKey)));
  ...
  async function x3dhAccept(chatId,senderId,bootstrap){
    if(!bootstrap?.x3dh)throw Error('missing X3DH bootstrap');const b=loadBundle();if(!b)throw Error('local X3DH private prekey bundle is unavailable');
    const signed=b.signedPreKey?.keyId===bootstrap.signedPreKeyId?b.signedPreKey:null;if(!signed)throw Error('signed prekey used by sender is no longer available');
    const one=(b.oneTimePreKeys||[]).find(k=>k.keyId===bootstrap.oneTimePreKeyId),my=window.KynectaE2E.getMyIdentityPrivateKey();
    const ika=await importPublic(bootstrap.identityPublicKey),eka=await importPublic(bootstrap.ephemeralPublicKey),sp=await importPrivate(signed.privateKey);
    const parts=[await dh(sp,ika),await dh(my,eka),await dh(sp,eka)];if(bootstrap.oneTimePreKeyId){if(!one)throw Error('one-time prekey already consumed');parts.push(await dh(await importPrivate(one.privateKey),eka))}
    const material=new Uint8Array(parts.reduce((n,p)=>n+p.byteLength,0));let off=0;for(const p of parts){material.set(new Uint8Array(p),off);off+=p.byteLength}
    const root=b64(await hkdf(material.buffer,'Kynecta-X3DH-v1-root',32));
    const initSend=b64(await hkdf(unb64(root),'Kynecta-X3DH-v1-init-send',32)),initRecv=b64(await hkdf(unb64(root),'Kynecta-X3DH-v1-init-recv',32));
  ...
  tryInitE2E();document.addEventListener('kyn:e2eUnlockRetry',()=>{provisioningPromise=null;tryInitE2E()});window.addEventListener('kyn:loggedIn',()=>{provisioningPromise=null;tryInitE2E()});
})();
