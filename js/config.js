...
(function () {
    ...
    // These two functions MUST exist synchronously before api.core.js/api.request.js execute.
    ...
    function applyBrand(root) {
        try {
            ...
            if (root.nodeType === Node.TEXT_NODE) {
                root.nodeValue = normalizeBrandText(root.nodeValue);
                return;
            }
            ...
            while ((node = walker.nextNode())) {
                node.nodeValue = normalizeBrandText(node.nodeValue);
            }
        } catch (_) {}
    }
    ...
    function removeCalls(root) {
        try {
            ...
            doc.querySelectorAll('button,a').forEach(function (node) {
                ...
                if (['calls', 'new call', 'voice call', 'video call'].indexOf(text) !== -1 || ['voice call', 'video call'].indexOf(title) !== -1) {
                    node.remove();
                }
            });
        } catch (_) {}
    ...
    function fixBrokenImages(root) {
        try {
            (root || document).querySelectorAll('img').forEach(function (img) {
                ...
                if (!img.getAttribute('src') || /\/undefined(?:$|[?#])/i.test(img.getAttribute('src'))) {
    ...
    window.__rewriteApiUrl = function (input) {
        ...
        function normalize(url) {
            ...
            if (/^\/api(?:\/|$)/i.test(url) || /^\/socket\.io(?:\/|$)/i.test(url) || /^\/ws(?:\/|$)/i.test(url)) {
                return origin + url;
            }
            try {
                var parsed = new URL(url, window.location.origin);
                if (/^\/api(?:\/|$)/i.test(parsed.pathname) || /^\/socket\.io(?:\/|$)/i.test(parsed.pathname) || /^\/ws(?:\/|$)/i.test(parsed.pathname)) {
        ...
        window.fetch = function (input, init) {
            try {
                ...
                if (/^\/api\/calls(?:\/|$)/i.test(pathname)) {
                    return Promise.resolve(new Response(JSON.stringify({ success: true, disabled: true, data: [], listings: [], message: 'Calling is disabled' }), {
                        status: 200,
    ...
    window.apiCall = async function (endpoint, options) {
        ...
        try {
            ...
            return await response.json().catch(function () { return { success: false, status: response.status }; });
        } catch (error) {
    ...
    function init() {
        ...
        if (/\/Tools\.html$/i.test(location.pathname) || /\/tools\.html$/i.test(location.pathname)) {
            ...
            loadOnce('/js/accommodation-marketplace-surface.js?v=20260916-5', 'accommodation_marketplace_surface');
            loadOnce('/js/invoice-ui.js?v=20260916-5', 'invoice_ui');
            loadOnce('/js/marketplace-image-hardening.js?v=20260916-5', 'marketplace_image_hardening');
        }
    }
