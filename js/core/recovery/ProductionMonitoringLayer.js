 ...
 *
 * Full production observability for Necpa:
...
(function () {
  ...
  class ProductionMonitoringLayer {
    ...
    start() {
      ...
      window.__NecpaDiag = () => this.printDiagnostics();
      window.__NecpaExport = () => this.exportJSON();

      console.log('[ProductionMonitor] ✅ Started — run __NecpaDiag() for full snapshot');
    }
    ...
    getSnapshot() {
      return {
        ...
        tombstones:  (() => { try { const t = JSON.parse(localStorage.getItem('necpa_tombstones_v1') || '{}'); return { count: Object.keys(t).length, ids: Object.keys(t) }; } catch(_) { return { count: 0 }; } })(),
    ...
    printDiagnostics() {
      ...
      console.group('🔍 Necpa Diagnostics — ' + snap.ts);
  ...
  console.log('[ProductionMonitor] ✅ Ready — __NecpaDiag() for full snapshot');
})();