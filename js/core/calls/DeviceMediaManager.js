...
(function () {
  ...
  class TrackLifecycleManager {
    ...
    register(stream) {
      ...
      stream.getTracks().forEach(t => this._tracks.add(t));
    }
    ...
    stopStream(stream) {
      if (!stream) return;
      stream.getTracks().forEach(t => this.stopTrack(t));
      this._streams.delete(stream);
    }

    stopAll() {
      for (const t of this._tracks)  { try { t.stop(); } catch (_) {} }
      this._tracks.clear();
      this._streams.clear();
    }
  ...
  class AudioOutputManager {
    ...
    async setSink(element, deviceId) {
    ...
    async routeToSpeaker(audioElements) {
      const devices = await this.getDevices();
      const speaker = devices.find(d => d.label.toLowerCase().includes('speaker'));
      if (!speaker) return;
      for (const el of audioElements) {
        await this.setSink(el, speaker.deviceId);
      }
    }

    async routeToEarpiece(audioElements) {
      const devices = await this.getDevices();
      const earpiece = devices.find(d =>
        d.label.toLowerCase().includes('earpiece') || d.label.toLowerCase().includes('internal')
      );
      if (!earpiece) return;
      for (const el of audioElements) {
        await this.setSink(el, earpiece.deviceId);
      }
    }
  }

  // ─── DeviceMediaManager (main) ────────────────────────────────────────────
  ...
  class DeviceMediaManager {
    constructor() {
      ...
      this._localStream  = null;   // The ONLY local stream — never shared as remote
    ...
    async acquireMedia(options = {}) {
      ...
      // Stop any existing local stream first — prevent camera LED staying on
      this._stopLocal();

      const constraints = {
        audio: audio ? {
          ...this._builder.audio(quality),
          ...(audioDeviceId ? { deviceId: { exact: audioDeviceId } } : {}),
        } : false,
        video: video ? {
          ...this._builder.video(quality),
      ...
      try {
        ...
        // Degrade: try audio-only if video failed
    ...
    async acquireScreen(options = {}) {
      ...
      try {
        ...
        stream.getVideoTracks()[0]?.addEventListener('ended', () => {
          this._stopScreen();
          this._notify('media:screen_stopped', {});
        });

        this._notify('media:screen_acquired', { stream });
        return stream;
      } catch (err) {
        console.warn('[DeviceMedia] Screen share failed:', err.message);
        throw err;
      }
    }
    ...
    muteAudio(muted) {
      ...
      this._muted = muted;
      this._notify('media:mute_changed', { muted });
    }

    toggleMute() { this.muteAudio(!this._muted); }
    isMuted()    { return this._muted; }

    disableVideo(disabled) {
      if (!this._localStream) return;
      this._localStream.getVideoTracks().forEach(t => { t.enabled = !disabled; });
      this._videoOff = disabled;
      this._notify('media:video_changed', { videoOff: disabled });
    }
    ...
    async switchCamera() {
      ...
      try {
        const newStream = await navigator.mediaDevices.getUserMedia({
          video: { ...this._builder.video(this._quality), facingMode: newFacing },
          audio: false,
        });
        ...
        console.warn('[DeviceMedia] Camera switch failed:', err.message);
        throw err;
      }
    }
    ...
    async switchAudioDevice(deviceId) {
      if (!this._localStream) return;
      const newStream = await navigator.mediaDevices.getUserMedia({
        audio: { ...this._builder.audio(this._quality), deviceId: { exact: deviceId } },
        video: false,
      });
    ...
    async adaptQuality(newQuality) {
      ...
      try {
        const profile = this._builder.video(newQuality);
        await videoTrack.applyConstraints(profile);
        this._notify('media:quality_changed', { quality: newQuality });
        console.log(`[DeviceMedia] Quality adapted to: ${newQuality}`);
      } catch (err) {
        console.warn('[DeviceMedia] applyConstraints failed:', err.message);
      }
    }
    ...
    async recoverTracks() {
      ...
      // Check if any tracks are ended (backgrounded/lost)
      ...
      if (!videoEnded && !audioEnded) return; // All good

      console.log('[DeviceMedia] Recovering ended tracks...');
      await this.acquireMedia({ audio: hasAudio, video: hasVideo && !this._videoOff, quality: this._quality });
    ...
    stopAll() {
      this._stopLocal();
      this._stopScreen();
      this._notify('media:stopped', {});
      console.log('[DeviceMedia] All media stopped');
    }
    ...
    getDiagnostics() {
      return {
        ...
        hasScreenStream: !!this._screenStream,
        muted:           this._muted,
        videoOff:        this._videoOff,
        quality:         this._quality,
        trackCount:      this._lifecycle.count(),
        localTracks:     this._localStream?.getTracks().map(t => ({ kind: t.kind, readyState: t.readyState, enabled: t.enabled })) || [],
      };
    }
    ...
    _stopLocal() {
      if (this._localStream) {
        this._lifecycle.stopStream(this._localStream);
    ...
    _stopScreen() {
      if (this._screenStream) {
        this._lifecycle.stopStream(this._screenStream);
        this._screenStream = null;
      }
    }
  ...
  // Auto-recover tracks on tab visibility restore
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
      setTimeout(() => window.__DeviceMediaManager.recoverTracks(), 500);
    }
  });
