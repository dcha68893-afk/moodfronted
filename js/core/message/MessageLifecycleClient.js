 ...
 *
 * IMPORTANT:
 * - `msg:new` is the canonical event.
 * - `message:new` is accepted only as a compatibility input because the deployed
 *   REST message path still emits it. It enters the EXACT SAME handler and is
 *   deduplicated by chatId:serverId; it is never rendered separately.
...
(function (global) {
  ...
  function installCryptoBootstrapRepair() {
    ...
    const wrapped = async function (plaintext, chatId, recipientId, opts) {
      ...
      try {
        ...
        env.x3dh = bootstrap;
        console.info('[E2E/X3DH] OUTGOING_BOOTSTRAP_REPAIRED', { chatId: String(chatId), recipientId: String(recipientId), sid: env.sid, n: env.n });
  ...
  function emitUI(message, plaintext) {
    ...
    const out = { ...message, content: plaintext, encrypted: false, status: message.status || 'delivered' };
    ...
    try {
      if (global.KynectaLocalStore && typeof global.KynectaLocalStore.saveMessage === 'function') {
        await global.KynectaLocalStore.saveMessage({
          ...
          chatId: String(message.chatId), conversationId: String(message.chatId),
          senderId: message.senderId, content: plaintext, type: message.type || 'text',
          sender: message.sender || null, replyToId: message.replyToId || null,
          createdAt: message.createdAt || Date.now(), status: 'delivered', isLocalOnly: false
        });
      }
    } catch (_) {}
  }
  ...
  function bind() {
    const socket = getSocket();
    if (!socket) { setTimeout(bind, 300); return; }
    if (boundSocket === socket) return;
    boundSocket = socket;
    // Canonical receive event.
    socket.on('msg:new', handleIncoming);
    // Compatibility receive event for the still-deployed REST/legacy sender.
    // It is intentionally routed into the same handler and therefore cannot
    // produce a second rendered message because handleIncoming deduplicates it.
    socket.on('message:new', handleIncoming);
    socket.on('msg:sync:result', data => { if (Array.isArray(data?.messages)) data.messages.forEach(handleIncoming); });
    socket.on('sync:missed_messages_result', data => { if (Array.isArray(data?.messages)) data.messages.forEach(handleIncoming); });
    socket.on('connect', () => { if (boundSocket !== getSocket()) { boundSocket = null; bind(); } });
  }

  function resetForAccountSwitch() {
    seen.clear(); decrypting.clear(); userId = null; cryptoRepairTarget = null;
  }
    ...
    return new Promise(resolve => {
      ...
      try {
        socket.emit('msg:send', payload, ackData => {
          if (done) return;
          done = true; clearTimeout(timer); resolve(ackData || { ok: false, reason: 'empty_ack' });
        });
      } catch (e) {
  ...
  global.MessageLifecycleClient = { init, sendMessage, sendViaSocket, markRead, requestSync, resetForAccountSwitch, _internal: { getSocket, handleIncoming } };
  ...
  init({ currentUserId: global.__PARENT_SESSION__?.userId || global.parent?.__PARENT_SESSION__?.userId || null }).catch(() => bind());
