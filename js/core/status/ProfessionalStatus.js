...
const stickers=['❤️','🔥','😂','😍','🎓','✨','💯','🙌','🎉','📚','🛍️','💡'];
try{const raw=JSON.parse(localStorage.getItem('necpa_status_seen_'+String(currentUser().id||'guest'))||'[]');window.__necpaStatusSeen=Array.isArray(raw)?raw:[]}catch(_){window.__necpaStatusSeen=[]}
let state={tab:'friends',statuses:[],mine:[],index:0,viewerGroup:[],composer:{type:'text',content:'',caption:'',background:bg[0],font:'system-ui',privacy:'all_contacts',topics:[],moodType:'',category:'',intent:'',allowReplies:true,allowReactions:true,allowSharing:true,linkUrl:'',mentions:[],stickers:[],media:null,poll:['',''],selectedSticker:''},seen:new Set(window.__necpaStatusSeen||[]),timer:null};

function syncTheme(){
 try{
  const source=(window.parent&&window.parent!==window)?window.parent.document:document;
  const srcRoot=source.documentElement, target=document.documentElement;
  const cs=source.defaultView.getComputedStyle(srcRoot);
  for(let i=0;i<cs.length;i++){const name=cs[i];if(name&&name.indexOf('--')===0){const value=cs.getPropertyValue(name);if(value)target.style.setProperty(name,value)}}
  const mode=srcRoot.getAttribute('data-theme');if(mode)target.setAttribute('data-theme',mode);
  const colorScheme=srcRoot.style.colorScheme;if(colorScheme)target.style.colorScheme=colorScheme;
 }catch(_){}
}
function mount(){
...
function renderPeople(){
 ...
 for(const s of state.statuses){
  const u=s.owner||{},id=String(u.id||s.userId);
  if(!groups[id])groups[id]=s;
  else if(groups[id].viewedByMe&&!s.viewedByMe)groups[id]=s;
 }
 ...
 el.innerHTML=arr.length?((fresh.length?'<div class="ns-status-label">NEW</div>'+fresh.map(person).join(''):'')+(viewed.length?'<div class="ns-status-label viewed-label">VIEWED</div>'+viewed.map(person).join(''):'')):'<div class="ns-empty-mini">No active friend statuses yet.</div>';
...
function renderFeed(){
 ...
 const section=(title,items,extra='')=>items.length?'<div class="ns-feed-section '+extra+'"><div class="ns-feed-title"><span>'+title+'</span><small>'+items.length+' moment'+(items.length===1?'':'s')+'</small></div><div class="ns-feed-grid">'+items.map((s,i)=>card(s,i)).join('')+'</div></div>':'';
...
function showViewer(){
 ...
 s.viewedByMe=true; state.seen.add(s.id);
 try{localStorage.setItem('necpa_status_seen_'+String(currentUser().id||'guest'),JSON.stringify([...state.seen].slice(-500)))}catch(_){}
 ...
 api('/view',{method:'POST',body:JSON.stringify({statusId:s.id})}).then(r=>{s.viewCount=Math.max(Number(s.viewCount||0),Number(s.viewCount||0)+(r.created?1:0));s.viewedByMe=true;renderFeed();renderPeople();}).catch(()=>{});
...
window.addEventListener('message',e=>{if(e.data?.type==='status:new'||e.data?.type==='STATUS_NEW'||e.data?.type==='FRIEND_STATUS_NEW')loadFeed()});
...
@media(max-width:800px){#necpa-status-root{padding:0!important}.ns-shell{grid-template-columns:1fr;min-height:100%;}.ns-side{display:none}.ns-main-head{padding:10px;gap:7px;min-height:54px}.ns-main-head .ns-search{min-width:0;width:0}.ns-main-head .ns-filter{max-width:105px}.ns-main-head .ns-btn{padding:9px 11px}.ns-feed{padding:12px 10px 20px}.ns-feed-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.ns-card{min-height:250px;border-radius:16px}.ns-card-top{top:10px;left:10px;right:10px}.ns-card-bottom{left:11px;right:11px;bottom:11px}.ns-card-caption{font-size:13px}.ns-composer{padding:0;align-items:stretch}.ns-compose-box{width:100%;height:100%;max-height:none;border-radius:0;padding:14px;overflow:auto}.ns-compose-tabs{grid-template-columns:repeat(4,minmax(0,1fr));position:sticky;top:0;z-index:2;background:var(--status-surface-2);padding-bottom:8px}.ns-compose-tab{padding:9px 4px;font-size:12px}.ns-textarea{min-height:150px}.ns-editor{min-height:230px}.ns-editor .ns-edit-text{font-size:25px}.ns-row{display:grid;grid-template-columns:1fr;gap:8px}.ns-input,.ns-select{width:100%;box-sizing:border-box}.ns-bg-grid,.ns-sticker-grid{grid-template-columns:repeat(5,1fr)}.ns-viewer-stage{width:100%}.ns-viewer-bottom{left:12px;right:12px;bottom:12px}.ns-viewer-head{left:12px;right:12px}.ns-viewer-actions button{width:36px;height:36px}.ns-nav{width:34px;height:34px}.ns-prev{left:6px}.ns-next{right:6px}}
