...
function showViewer(){
 ...
 root.classList.add('open');try{window.parent.postMessage({type:'STATUS_VIEW_OPENED',source:'professional-status'},'*')}catch(_){}
...
function closeViewer(){clearInterval(state.timer);const el=document.querySelector('[data-viewer]');if(el?.classList.contains('open')){el.classList.remove('open');try{window.parent.postMessage({type:'STATUS_LIST_SHOWN',source:'professional-status'},'*')}catch(_){}}}
...
function openComposer(){
 closeViewer();try{window.parent.postMessage({type:'STATUS_CREATE_OPENED',source:'professional-status'},'*')}catch(_){}const root=document.querySelector('[data-composer]');const c=state.composer={type:'text',content:'',caption:'',background:bg[0],font:'system-ui',privacy:'all_contacts',topics:[],moodType:'',category:'',intent:'',allowReplies:true,allowReactions:true,allowSharing:true,linkUrl:'',mentions:[],stickers:[],media:null,poll:['',''],selectedSticker:''};
...
function closeComposer(){const el=document.querySelector('[data-composer]');if(el?.classList.contains('open')){el.classList.remove('open');try{window.parent.postMessage({type:'STATUS_LIST_SHOWN',source:'professional-status'},'*')}catch(_){}}}
