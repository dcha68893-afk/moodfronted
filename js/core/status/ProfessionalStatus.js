...
let vibesState={items:[],index:0,muted:true,onlySaved:false,mode:'forYou',speed:1,searchQuery:'',loading:false};
window.vibesState=vibesState;
const VIBE_MODES={forYou:'For You',following:'Following',friends:'Friends',public:'Public'};
const vibeHiddenKey=()=> 'necpa_vibe_hidden_'+String(currentUser().id||'guest');
async function vibesSource(mode=vibesState.mode||'forYou',q=''){
 ...
 let hidden=new Set();try{hidden=new Set(JSON.parse(localStorage.getItem(vibeHiddenKey())||'[]').map(String))}catch(_){}
 return data.map(normalizeStatus).filter(s=>s.type==='video'&&s.mediaUrl&&!hidden.has(String(s.id)));
}
function vibeStopMedia(root){root?.querySelectorAll('video,audio').forEach(v=>{try{v.pause();v.muted=true;v.removeAttribute('src');v.load()}catch(_){} });}
function vibeCurrent(){return vibesState.items[vibesState.index]||null}
function vibeShell(root,body){root.innerHTML='<div class="nv-app">'+body+'</div>';root.querySelectorAll('[data-vibe-action]').forEach(btn=>btn.addEventListener('click',e=>handleVibeAction(e,btn.dataset.vibeAction)))}
function vibeTop(){return '<header class="nv-top"><button class="nv-round" data-vibe-action="close" aria-label="Close Vibes">×</button><div class="nv-brand"><strong>Vibes</strong><span>Short moments. Real connections.</span></div><nav class="nv-tabs" aria-label="Vibe feeds">'+Object.entries(VIBE_MODES).map(([k,v])=>'<button class="'+(k===vibesState.mode?'active':'')+'" data-vibe-action="tab" data-mode="'+k+'">'+v+'</button>').join('')+'</nav><div class="nv-top-actions"><button class="nv-round" data-vibe-action="search" aria-label="Search Vibes">⌕</button><button class="nv-round" data-vibe-action="saved" aria-label="Saved Vibes">▱</button></div></header>'}
function vibeSearchPanel(){return '<div class="nv-search-panel" data-vibe-search-panel><form data-vibe-search-form><input data-vibe-search-input value="'+esc(vibesState.searchQuery||'')+'" placeholder="Search creators, captions and topics" autocomplete="off"><button type="submit">Search</button><button type="button" data-vibe-action="search-close" class="nv-search-close">×</button></form></div>'}
function vibeEmpty(title,message){return vibeTop()+'<main class="nv-empty"><div class="nv-empty-mark">◌</div><h2>'+esc(title)+'</h2><p>'+esc(message)+'</p><button class="nv-primary" data-vibe-action="close">Back</button></main>'}
async function openVibes(){const root=document.querySelector('[data-vibes]');if(!root)return;clearInterval(state.timer);root.classList.add('open');vibesState={items:[],index:0,muted:true,onlySaved:false,mode:'forYou',speed:1,searchQuery:'',loading:true};window.vibesState=vibesState;vibeShell(root,vibeTop()+'<main class="nv-loading"><div class="nv-spinner"></div><strong>Loading Vibes</strong><span>Finding something worth watching…</span></main>');syncParent();await loadVibeFeed('forYou')}
function closeVibes(){const root=document.querySelector('[data-vibes]');if(!root?.classList.contains('open'))return;flushCurrentVibeWatch();vibeStopMedia(root);root.classList.remove('open');root.innerHTML='';vibesState={items:[],index:0,muted:true,onlySaved:false,mode:'forYou',speed:1,searchQuery:'',loading:false};window.vibesState=vibesState;syncParent()}
async function loadVibeFeed(mode=vibesState.mode||'forYou',q=''){const root=document.querySelector('[data-vibes]');if(!root?.classList.contains('open'))return;vibesState.loading=true;vibesState.mode=VIBE_MODES[mode]?mode:'forYou';vibesState.searchQuery=q||'';vibeShell(root,vibeTop()+'<main class="nv-loading"><div class="nv-spinner"></div><strong>Loading '+esc(VIBE_MODES[vibesState.mode])+'</strong><span>Preparing your next Vibe…</span></main>');try{const items=await vibesSource(vibesState.mode,q);if(!root.classList.contains('open'))return;vibesState={...vibesState,items,index:0,loading:false,onlySaved:false};window.vibesState=vibesState;if(items.length)renderVibe();else vibeShell(root,vibeEmpty(q?'No matching Vibes':'Nothing here yet',q?'Try another creator, topic or phrase.':'New Vibes will appear here as people post.'))}catch(e){vibesState.loading=false;window.vibesState=vibesState;vibeShell(root,vibeTop()+'<main class="nv-empty"><div class="nv-empty-mark">!</div><h2>Vibes could not load</h2><p>'+esc(e.message||'Please try again.')+'</p><button class="nv-primary" data-vibe-action="retry">Try again</button></main>')}}
async function switchVibeTab(mode){if(VIBE_MODES[mode]){flushCurrentVibeWatch();await loadVibeFeed(mode,vibesState.searchQuery||'')}}
async function searchVibes(){const root=document.querySelector('[data-vibes]');if(!root?.classList.contains('open'))return;const panel=root.querySelector('[data-vibe-search-panel]');if(panel){root.querySelector('[data-vibe-search-input]')?.focus();return}root.querySelector('.nv-app')?.insertAdjacentHTML('beforeend',vibeSearchPanel());root.querySelector('[data-vibe-search-input]')?.focus();root.querySelector('[data-vibe-search-form]')?.addEventListener('submit',async e=>{e.preventDefault();const q=(root.querySelector('[data-vibe-search-input]')?.value||'').trim();root.querySelector('[data-vibe-search-panel]')?.remove();if(q)await loadVibeFeed(vibesState.mode,q);else renderVibe()})}
function toggleSavedVibesView(){const saved=loadSavedVibeIds();if(!saved.size){toast('No saved Vibes yet');return}const items=(vibesState.items||[]).filter(s=>saved.has(String(s.id)));if(!items.length){toast('Saved Vibes are not in this feed');return}flushCurrentVibeWatch();vibesState={...vibesState,items,index:0,onlySaved:true,searchQuery:''};window.vibesState=vibesState;renderVibe()}
function renderVibe(){const root=document.querySelector('[data-vibes]');if(!root)return;vibeStopMedia(root);const s=vibeCurrent();if(!s){closeVibes();return}const u=s.owner||{},saved=loadSavedVibeIds().has(String(s.id)),isMine=String(u.id||s.userId)===String(currentUser().id||''),progress=Math.round(((vibesState.index+1)/Math.max(1,vibesState.items.length))*100),next=vibesState.items[vibesState.index+1];const body=vibeTop()+'<main class="nv-stage" data-rstage><div class="nv-progress"><i style="width:'+progress+'%"></i></div><div class="nv-media-backdrop"></div><video class="nv-video" src="'+esc(String(s.mediaUrl||''))+'" playsinline autoplay muted preload="auto" poster="'+esc(String(s.thumbnailUrl||s.thumbnail||''))+'" aria-label="Vibe video"></video><div class="nv-media-loading" data-vibe-media-loading><div class="nv-spinner"></div><span>Preparing video…</span></div><div class="nv-media-error" data-vibe-media-error hidden><strong>This Vibe could not be loaded.</strong><button data-vibe-action="media-retry">Try again</button></div><button class="nv-tap-play" data-vibe-action="toggle-play" aria-label="Play or pause"></button><div class="nv-gradient nv-gradient-top"></div><div class="nv-gradient nv-gradient-bottom"></div><section class="nv-copy"><div class="nv-creator">'+avatar(u,'nv-avatar')+'<div class="nv-creator-name"><b>'+esc(u.displayName||u.username||'Creator')+'</b><small>@'+esc(u.username||'creator')+'</small></div>'+(isMine?'':'<button class="nv-follow '+(s.isFollowedByMe?'following':'')+'" data-vibe-action="follow">'+(s.isFollowedByMe?'Following':'Follow')+'</button>')+'</div><p class="nv-caption">'+esc(s.caption||s.content||'')+'</p><div class="nv-tags">'+(Array.isArray(s.topics)?s.topics.slice(0,5).map(t=>'<span>#'+esc(t)+'</span>').join(''):'')+'</div><div class="nv-sound"><span>◖</span><span>'+esc(s.musicTitle||s.musicName||s.audioTitle||'Original sound')+'</span></div></section><aside class="nv-actions" aria-label="Vibe actions"><button class="nv-action nv-avatar-action" data-vibe-action="profile" aria-label="Open creator profile"><b>'+avatar(u,'nv-action-avatar')+'</b></button><button class="nv-action '+(s.likedByMe?'liked':'')+'" data-vibe-action="like" aria-label="Love Vibe"><b>♥</b><span data-vibe-like-count>'+compactCount(s.reactionCount||0)+'</span></button><button class="nv-action" data-vibe-action="comments" aria-label="Comments"><b>◌</b><span data-vibe-comment-count>'+compactCount(s.replyCount||0)+'</span></button><button class="nv-action '+(saved?'active':'')+'" data-vibe-action="save" aria-label="Save Vibe"><b>▱</b><span>'+(saved?'Saved':'Save')+'</span></button><button class="nv-action" data-vibe-action="download" aria-label="Download Vibe"><b>↓</b><span>Save video</span></button><button class="nv-action" data-vibe-action="share" aria-label="Share Vibe"><b>↗</b><span>Share</span></button><button class="nv-action" data-vibe-action="mute" aria-label="Mute or unmute"><b data-vibe-mute-icon>'+(vibesState.muted?'◖':'◉')+'</b><span>'+(vibesState.muted?'Sound':'Mute')+'</span></button><button class="nv-action" data-vibe-action="more" aria-label="More Vibe actions"><b>•••</b><span>More</span></button></aside><div class="nv-count"><strong>'+compactCount(s.viewCount||0)+'</strong> views <span>·</span> '+(vibesState.index+1)+' / '+vibesState.items.length+'</div></main><div class="nv-comments" data-vibe-comments></div><div class="nv-more" data-vibe-more hidden></div>';vibeShell(root,body);const v=root.querySelector('.nv-video');if(v){v.muted=vibesState.muted;v.playbackRate=Number(vibesState.speed||1);v.addEventListener('loadeddata',()=>root.querySelector('[data-vibe-media-loading]')?.setAttribute('hidden','hidden'),{once:true});v.addEventListener('waiting',()=>root.querySelector('[data-vibe-media-loading]')?.removeAttribute('hidden'));v.addEventListener('playing',()=>root.querySelector('[data-vibe-media-loading]')?.setAttribute('hidden','hidden'));v.addEventListener('error',()=>{root.querySelector('[data-vibe-media-loading]')?.setAttribute('hidden','hidden');root.querySelector('[data-vibe-media-error]')?.removeAttribute('hidden')});v.addEventListener('timeupdate',()=>trackVibeWatch(s,v));v.addEventListener('ended',()=>{flushCurrentVibeWatch();if(vibesState.index<vibesState.items.length-1)moveVibe(1)});v.play().catch(()=>{})}if(next?.mediaUrl){const pre=document.createElement('link');pre.rel='preload';pre.as='video';pre.href=next.mediaUrl;pre.dataset.vibePreload='1';root.appendChild(pre)}wireVibeGestures(root);recordVibeView(s)}
function handleVibeAction(e,action){const root=document.querySelector('[data-vibes]');if(!root)return;e?.preventDefault?.();e?.stopPropagation?.();const s=vibeCurrent();if(action==='close')return closeVibes();if(action==='retry')return loadVibeFeed(vibesState.mode,vibesState.searchQuery);if(action==='tab')return switchVibeTab(e.currentTarget.dataset.mode);if(action==='search')return searchVibes();if(action==='search-close')return root.querySelector('[data-vibe-search-panel]')?.remove();if(action==='saved')return toggleSavedVibesView();if(action==='toggle-play'){const v=root.querySelector('.nv-video');if(v){if(v.paused)v.play().catch(()=>{});else v.pause()}return}if(action==='media-retry'){const v=root.querySelector('.nv-video');if(v){v.load();v.play().catch(()=>{})}return}if(!s)return;if(action==='like')return toggleVibeLove(s);if(action==='comments')return openVibeComments(s);if(action==='save'){const set=loadSavedVibeIds(),id=String(s.id);if(set.has(id)){set.delete(id);toast('Removed from Saved')}else{set.add(id);toast('Saved to your Vibes')}persistSavedVibeIds(set);return renderVibe()}if(action==='download'){try{const a=document.createElement('a');a.href=s.mediaUrl;a.download='vibe-'+s.id+'.mp4';a.target='_blank';a.rel='noopener';document.body.appendChild(a);a.click();a.remove();toast('Download started')}catch(_){toast('Download unavailable')}return}if(action==='share')return share(s);if(action==='mute'){vibesState.muted=!vibesState.muted;const v=root.querySelector('.nv-video');if(v){v.muted=vibesState.muted;if(!v.muted)v.volume=.9}const icon=root.querySelector('[data-vibe-mute-icon]');if(icon)icon.textContent=vibesState.muted?'◖':'◉';const label=icon?.parentElement?.querySelector('span');if(label)label.textContent=vibesState.muted?'Sound':'Mute';return}if(action==='follow')return toggleVibeFollow(s);if(action==='profile'){const id=Number((s.owner||{}).id||s.userId);try{window.parent?.postMessage({type:'OPEN_USER_PROFILE',userId:id},'*')}catch(_){}window.dispatchEvent(new CustomEvent('necpa:open-profile',{detail:{userId:id}}));return}if(action==='more')return openVibeMore(s)}
function openVibeMore(s){const root=document.querySelector('[data-vibes]'),box=root?.querySelector('[data-vibe-more]');if(!root||!box)return;box.hidden=!box.hidden;if(box.hidden)return;box.innerHTML='<div class="nv-more-card"><button data-more="not">Not interested</button><button data-more="report">Report Vibe</button><button data-more="copy">Copy link</button><button data-more="speed">Playback speed</button></div>';box.querySelector('[data-more="not"]').onclick=()=>{try{const ids=new Set(JSON.parse(localStorage.getItem(vibeHiddenKey())||'[]').map(String));ids.add(String(s.id));localStorage.setItem(vibeHiddenKey(),JSON.stringify([...ids].slice(-1000)))}catch(_){}box.hidden=true;moveVibe(1)};box.querySelector('[data-more="report"]').onclick=async()=>{const reason=prompt('Why are you reporting this Vibe?','spam');if(!reason)return;try{await api('/'+s.id+'/report',{method:'POST',body:JSON.stringify({reason})});toast('Report submitted')}catch(e){toast(e.message||'Could not submit report')}box.hidden=true};box.querySelector('[data-more="copy"]').onclick=async()=>{try{await navigator.clipboard.writeText(location.origin+'/status.html?vibe='+encodeURIComponent(s.id));toast('Vibe link copied')}catch(_){toast('Could not copy link')}box.hidden=true};box.querySelector('[data-more="speed"]').onclick=()=>{const speeds=[1,.75,1.25,1.5,2],i=speeds.indexOf(Number(vibesState.speed||1));vibesState.speed=speeds[(i+1)%speeds.length];const v=root.querySelector('.nv-video');if(v)v.playbackRate=vibesState.speed;toast('Playback speed '+vibesState.speed+'×')}}
async function sendVibeFriendRequest(u){const id=Number(u?.id||0);if(!id||id===Number(currentUser().id||0))return;try{const r=await fetch(apiOrigin()+'/api/friends/requests',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+token()},body:JSON.stringify({userId:id})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.message||'Unable to send request');toast('Friend request sent')}catch(e){toast(e.message||'Unable to send request')}}
async function toggleVibeFollow(s){const u=s.owner||{},id=Number(u.id||0);if(!id||id===Number(currentUser().id||0)||s.__following)return;s.__following=true;const was=!!s.isFollowedByMe,wasCount=Number(u.followerCount)||0;s.isFollowedByMe=!was;if(s.owner)s.owner.followerCount=Math.max(0,wasCount+(s.isFollowedByMe?1:-1));paintVibe(s);try{const r=await fetch(apiOrigin()+'/api/profile/'+id+'/follow',{method:was?'DELETE':'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+token()}});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.message||'Could not update follow status');if(d.data?.following!==undefined)s.isFollowedByMe=!!d.data.following;if(d.data?.followerCount!=null&&s.owner)s.owner.followerCount=Number(d.data.followerCount)||0;toast(s.isFollowedByMe?'Following '+(u.displayName||u.username||'creator'):'Unfollowed')}catch(e){s.isFollowedByMe=was;if(s.owner)s.owner.followerCount=wasCount;toast(e.message||'Could not update follow status')}finally{s.__following=false;paintVibe(s)}}
...
const vibeMeta=(s,expiry)=>compactCount(s.owner?.followerCount||0)+' followers · '+compactCount(s.viewCount||0)+' views'+(expiry?' · '+expiry:'');
function paintVibe(s){if(vibesState.items[vibesState.index]!==s)return;const root=document.querySelector('[data-vibes]');if(!root)return;const lc=root.querySelector('[data-vibe-like-count]');if(lc)lc.textContent=compactCount(s.reactionCount||0);const cc=root.querySelector('[data-vibe-comment-count]');if(cc)cc.textContent=compactCount(s.replyCount||0);const like=root.querySelector('[data-vibe-action="like"]');if(like)like.classList.toggle('liked',!!s.likedByMe);const follow=root.querySelector('[data-vibe-action="follow"]');if(follow){follow.classList.toggle('following',!!s.isFollowedByMe);follow.textContent=s.isFollowedByMe?'Following':'Follow'}}
async function toggleVibeLove(s){if(s.__loving)return;s.__loving=true;const was=!!s.likedByMe,wc=Number(s.reactionCount||0);s.likedByMe=!was;s.reactionCount=Math.max(0,wc+(s.likedByMe?1:-1));paintVibe(s);try{const r=await api('/vibes/'+s.id+'/love',{method:'POST',body:'{}'});if(r?.liked!==undefined)s.likedByMe=!!r.liked;if(r?.count!=null)s.reactionCount=Number(r.count)||0}catch(e){s.likedByMe=was;s.reactionCount=wc;toast(e.message||'Could not update your love')}finally{s.__loving=false;paintVibe(s)}}
const __viewedVibes=new Set();
function recordVibeView(s){const id=String(s.id);if(__viewedVibes.has(id))return;__viewedVibes.add(id);if(String(s.userId)===String(currentUser().id||''))return;api('/view',{method:'POST',body:JSON.stringify({statusId:s.id})}).then(r=>{const d=(r&&r.data)||r||{};if(d.viewCount!=null){s.viewCount=Number(d.viewCount)||0;paintVibe(s)}}).catch(()=>__viewedVibes.delete(id))}
function trackVibeWatch(s,v){const now=Number(v.currentTime)||0,w=s.__watch||(s.__watch={secondsWatched:0,playCount:1,completionCount:0,rewatchCount:0,lastTime:now,flushed:0,done:false});const delta=now-Number(w.lastTime||0);if(delta>0&&delta<2.5)w.secondsWatched+=delta;if(now<.35&&Number(w.lastTime||0)>1)w.rewatchCount=(w.rewatchCount||0)+1;if(v.duration&&now>=v.duration-.4&&!w.done){w.done=true;w.completionCount=(w.completionCount||0)+1}w.lastTime=now}
function flushVibeWatch(s){const w=s?.__watch;if(!s||!w)return;const total=Number(w.secondsWatched||0);if(total<=0)return;if(Number(w.flushed||0)>=total&&w.flushedDone===!!w.done)return;w.flushed=total;w.flushedDone=!!w.done;api('/vibes/'+s.id+'/watch',{method:'POST',body:JSON.stringify({secondsWatched:Math.min(total,3600),playCount:Math.min(Number(w.playCount)||1,100),completionCount:Math.min(Number(w.completionCount)||0,100),rewatchCount:Math.min(Number(w.rewatchCount)||0,100)})}).catch(()=>{})}
function flushCurrentVibeWatch(){const s=vibeCurrent();if(s)flushVibeWatch(s)}
async function openVibeComments(s){const root=document.querySelector('[data-vibe-comments]');if(!root)return;root.classList.add('open');root.innerHTML='<div class="nv-comments-card"><header><strong>Comments</strong><button data-vcclose>×</button></header><div class="nv-comment-list">Loading…</div><form class="nv-comment-compose"><input data-vctext maxlength="2000" placeholder="Add a thoughtful comment…"><button>Send</button></form></div>';const list=root.querySelector('.nv-comment-list'),input=root.querySelector('[data-vctext]');root.querySelector('[data-vcclose]').onclick=()=>root.classList.remove('open');const name=x=>{const u=x.user||{};return u.displayName||u.username||(String(x.userId)===String(currentUser().id||'')?'You':'User '+x.userId)},row=x=>'<article><b>'+esc(name(x))+'</b><span>'+esc(x.text||'')+'</span></article>',paint=rows=>{list.innerHTML=rows.length?rows.map(row).join(''):'<div class="nv-no-comments">No comments yet. Start the conversation.</div>'};try{const r=await api('/'+s.id+'/comments');const rows=Array.isArray(r.data)?r.data:[];paint(rows);if(r.count!=null)s.replyCount=Number(r.count)||0;paintVibe(s)}catch(e){list.textContent=e.message||'Could not load comments'}root.querySelector('.nv-comment-compose').onsubmit=async e=>{e.preventDefault();const text=(input.value||'').trim();if(!text)return;const btn=e.currentTarget.querySelector('button');btn.disabled=true;try{const r=await api('/'+s.id+'/comment',{method:'POST',body:JSON.stringify({text})});s.replyCount=r.count!=null?Number(r.count):(Number(s.replyCount)||0)+1;input.value='';const fresh=await api('/'+s.id+'/comments').catch(()=>null);paint(fresh&&Array.isArray(fresh.data)?fresh.data:[{text,userId:currentUser().id}]);paintVibe(s)}catch(err){toast(err.message||'Could not post comment')}finally{btn.disabled=false;input.focus()}}}
function moveVibe(dir){if(!document.querySelector('[data-vibes]')?.classList.contains('open'))return;flushCurrentVibeWatch();const n=vibesState.index+dir;if(n<0||n>=vibesState.items.length)return;vibesState.index=n;window.vibesState=vibesState;renderVibe()}
function wireVibeGestures(root){const stage=root.querySelector('[data-rstage]');if(!stage)return;let startY=null,startX=null;stage.addEventListener('touchstart',e=>{const t=e.touches[0];startY=t.clientY;startX=t.clientX},{passive:true});stage.addEventListener('touchend',e=>{if(startY==null)return;const t=e.changedTouches[0],dy=t.clientY-startY,dx=t.clientX-startX;startY=null;if(Math.abs(dy)<55||Math.abs(dy)<Math.abs(dx))return;moveVibe(dy<0?1:-1)},{passive:true});stage.addEventListener('wheel',e=>{if(Math.abs(e.deltaY)<30)return;e.preventDefault();moveVibe(e.deltaY>0?1:-1)},{passive:false});stage.addEventListener('click',e=>{if(e.target.closest('button,.nv-actions,.nv-copy'))return;const v=stage.querySelector('.nv-video');if(!v)return;if(v.paused)v.play().catch(()=>{});else v.pause()})}

const vibeDraftsKey=()=> 'necpa_vibe_drafts_'+String(currentUser().id||'guest');
function loadVibeDrafts(){try{return JSON.parse(localStorage.getItem(vibeDraftsKey())||'[]')}catch(_){return[]}}
function saveVibeDraft(d){const rows=loadVibeDrafts().filter(x=>String(x.id)!==String(d.id));rows.unshift(Object.assign({id:'d_'+Date.now(),savedAt:Date.now()},d));try{localStorage.setItem(vibeDraftsKey(),JSON.stringify(rows.slice(0,20)))}catch(_){}toast('Draft saved');return rows[0]}
function deleteVibeDraft(id){const rows=loadVibeDrafts().filter(x=>String(x.id)!==String(id));try{localStorage.setItem(vibeDraftsKey(),JSON.stringify(rows))}catch(_){}}
function draftSnapshot(){
 const root=document.querySelector('[data-composer]'),c=state.composer||{};
 return {type:c.type,content:root?.querySelector('[data-content]')?.value||c.content||'',caption:c.caption||'',background:c.background,font:c.font,privacy:root?.querySelector('[data-privacy]')?.value||c.privacy,duration:root?.querySelector('[data-duration]')?.value||7,mood:root?.querySelector('[data-mood]')?.value||'',category:root?.querySelector('[data-category]')?.value||'',intent:root?.querySelector('[data-intent]')?.value||'',topics:root?.querySelector('[data-topics]')?.value||'',music:root?.querySelector('[data-music]')?.value||'',publicationTarget:root?.querySelector('[data-publication-target]')?.value||'status',vibeDuration:root?.querySelector('[data-vibe-duration]')?.value||24,stickers:c.stickers||[]};
}
function openVibeDrafts(){
 const root=document.querySelector('[data-composer]');if(!root)return;
 const rows=loadVibeDrafts();
 const box=document.createElement('div');box.className='ns-drafts-modal';
 box.innerHTML='<div class="ns-drafts-box"><div class="ns-compose-head"><div><h3>Drafts</h3><small>Saved on this device and account</small></div><button class="ns-icon-btn" data-dclose>×</button></div><div class="ns-draft-list">'+(rows.length?rows.map(d=>'<button class="ns-draft-row" data-draft="'+esc(d.id)+'"><b>'+esc((d.content||'Video draft').slice(0,70))+'</b><small>'+new Date(d.savedAt||Date.now()).toLocaleString()+'</small><i data-ddel="'+esc(d.id)+'">Delete</i></button>').join(''):'<div class="ns-empty">No drafts yet.</div>')+'</div></div>';
 document.body.appendChild(box);box.querySelector('[data-dclose]').onclick=()=>box.remove();
 box.querySelectorAll('[data-ddel]').forEach(b=>b.onclick=e=>{e.stopPropagation();deleteVibeDraft(b.dataset.ddel);box.remove();openVibeDrafts()});
 box.querySelectorAll('[data-draft]').forEach(b=>b.onclick=()=>{const d=rows.find(x=>String(x.id)===String(b.dataset.draft));if(!d)return;box.remove();openComposer();setTimeout(()=>restoreVibeDraft(d),0)});
}
function restoreVibeDraft(d){
 const root=document.querySelector('[data-composer]');if(!root)return;
 const c=state.composer;c.type=d.type||'text';c.content=d.content||'';c.caption=d.caption||'';c.background=d.background||bg[0];c.font=d.font||'system-ui';c.stickers=d.stickers||[];c.publicationTarget=d.publicationTarget||'status';c.vibeDurationHours=Number(d.vibeDuration)||24;
 const q=(s)=>root.querySelector(s);
 if(q('[data-content]'))q('[data-content]').value=c.content;if(q('[data-edittext]'))q('[data-edittext]').textContent=c.content;
 if(q('[data-caption]'))q('[data-caption]').value=c.caption;if(q('[data-privacy]'))q('[data-privacy]').value=d.privacy||'all_contacts';if(q('[data-duration]'))q('[data-duration]').value=d.duration||7;
 if(q('[data-mood]'))q('[data-mood]').value=d.mood||'';if(q('[data-category]'))q('[data-category]').value=d.category||'';if(q('[data-intent]'))q('[data-intent]').value=d.intent||'';if(q('[data-topics]'))q('[data-topics]').value=d.topics||'';if(q('[data-music]'))q('[data-music]').value=d.music||'';if(q('[data-publication-target]'))q('[data-publication-target]').value=c.publicationTarget;if(q('[data-vibe-duration]'))q('[data-vibe-duration]').value=c.vibeDurationHours;
 q('[data-editor]')?.style.setProperty('background',c.background);q('[data-editor]')?.style.setProperty('fontFamily',c.font);
 toast('Draft restored. Re-select media if this draft used a video or photo.');
}
...
function wireComposer(){
 const root=document.querySelector('[data-composer]');root.querySelector('[data-cclose]').onclick=closeComposer;root.querySelector('[data-drafts]')?.addEventListener('click',openVibeDrafts);root.querySelector('[data-save-draft]')?.addEventListener('click',()=>{saveVibeDraft(draftSnapshot());});root.querySelector('[data-cback]').onclick=closeComposer;root.querySelector('[data-cancel]').onclick=closeComposer;
 ...
 root.querySelector('[data-rotate]').onclick=()=>{const p=root.querySelector('.ns-preview');if(!p)return;const n=(Number(p.dataset.rot||0)+90)%360;p.dataset.rot=n;p.style.transform='rotate('+n+'deg)';state.composer.media&&(state.composer.media.rotation=n);};
 root.querySelector('[data-zoomout]').onclick=()=>{const p=root.querySelector('.ns-preview');if(!p)return;const n=Math.max(.7,Number(p.dataset.zoom||1)-.1);p.dataset.zoom=n;p.style.transform='scale('+n+')';};
 root.querySelector('[data-zoomin]').onclick=()=>{const p=root.querySelector('.ns-preview');if(!p)return;const n=Math.min(1.5,Number(p.dataset.zoom||1)+.1);p.dataset.zoom=n;p.style.transform='scale('+n+')';};
 root.querySelector('[data-filter]').onclick=()=>{const p=root.querySelector('.ns-preview');if(!p)return;const filters=['none','contrast(1.15) saturate(1.15)','grayscale(.85)','sepia(.35) saturate(1.2)'];const i=(Number(p.dataset.filter||0)+1)%filters.length;p.dataset.filter=i;p.style.filter=filters[i];};
...
(function(){const s=document.createElement('style');s.id='necpa-vibe-platform-ui';s.textContent='#necpa-status-root .ns-vibes.open{background:var(--kyn-bg-root,#08090d)!important;color:var(--kyn-text-primary,#fff)!important;pointer-events:auto!important}#necpa-status-root .nv-app{position:absolute;inset:0;overflow:hidden;background:var(--kyn-bg-root,#08090d);color:var(--kyn-text-primary,#fff);font-family:inherit}#necpa-status-root .nv-top{position:absolute;z-index:30;top:0;left:0;right:0;height:76px;display:grid;grid-template-columns:auto 180px minmax(0,1fr) auto;align-items:center;gap:18px;padding:12px 18px;padding-top:calc(12px + env(safe-area-inset-top,0px));background:linear-gradient(180deg,rgba(0,0,0,.72),rgba(0,0,0,0));pointer-events:none}#necpa-status-root .nv-top>*{pointer-events:auto}#necpa-status-root .nv-round{width:42px;height:42px;border:1px solid rgba(255,255,255,.18);border-radius:50%;display:grid;place-items:center;background:rgba(0,0,0,.42);color:#fff;font-size:23px;cursor:pointer;backdrop-filter:blur(16px);transition:transform .16s ease,background .16s ease}#necpa-status-root .nv-round:hover{background:rgba(255,255,255,.16)}#necpa-status-root .nv-round:active{transform:scale(.9)}#necpa-status-root .nv-brand{display:flex;flex-direction:column;line-height:1}.nv-brand strong{font-size:20px;letter-spacing:-.03em}.nv-brand span{font-size:10px;opacity:.62;margin-top:5px;white-space:nowrap}#necpa-status-root .nv-tabs{display:flex;justify-content:center;gap:22px;min-width:0;overflow-x:auto;scrollbar-width:none}.nv-tabs::-webkit-scrollbar{display:none}#necpa-status-root .nv-tabs button{border:0;background:none;color:rgba(255,255,255,.58);font:700 13px inherit;padding:12px 2px;cursor:pointer;position:relative;white-space:nowrap}.nv-tabs button.active{color:#fff}.nv-tabs button.active:after{content:"";position:absolute;left:18%;right:18%;bottom:3px;height:3px;border-radius:9px;background:var(--status-accent,#7c3aed)}#necpa-status-root .nv-top-actions{display:flex;gap:8px}#necpa-status-root .nv-stage{position:absolute;inset:0;display:flex;justify-content:center;align-items:center;overflow:hidden;background:#050609}#necpa-status-root .nv-video{position:relative;z-index:2;height:100%;width:auto;max-width:100%;object-fit:contain;background:#050609;box-shadow:0 0 80px rgba(0,0,0,.55)}#necpa-status-root .nv-media-backdrop{position:absolute;inset:-30px;background:radial-gradient(circle at 50% 45%,rgba(124,58,237,.2),transparent 45%);filter:blur(32px);opacity:.6;z-index:0}#necpa-status-root .nv-progress{position:absolute;z-index:8;top:82px;left:18px;right:18px;height:3px;background:rgba(255,255,255,.22);border-radius:9px;overflow:hidden}.nv-progress i{display:block;height:100%;background:#fff;border-radius:9px;transition:width .2s ease}#necpa-status-root .nv-gradient{position:absolute;left:0;right:0;z-index:4;pointer-events:none}.nv-gradient-top{top:0;height:170px;background:linear-gradient(180deg,rgba(0,0,0,.38),transparent)}.nv-gradient-bottom{bottom:0;height:42%;background:linear-gradient(0deg,rgba(0,0,0,.76),transparent)}#necpa-status-root .nv-copy{position:absolute;z-index:7;left:max(22px,calc(50% - 290px));bottom:26px;width:min(520px,calc(100% - 150px));pointer-events:none}.nv-creator{display:flex;align-items:center;gap:10px;margin-bottom:12px;pointer-events:auto}.nv-avatar{width:44px!important;height:44px!important;border-radius:50%;border:2px solid rgba(255,255,255,.9);background-size:cover;background-position:center;flex:none}.nv-creator-name{min-width:0}.nv-creator-name b{display:block;font-size:14px}.nv-creator-name small{display:block;font-size:11px;opacity:.65;margin-top:3px}.nv-follow{border:1px solid rgba(255,255,255,.65);border-radius:999px;background:rgba(255,255,255,.12);color:#fff;padding:7px 13px;font:800 11px inherit;cursor:pointer;backdrop-filter:blur(10px)}.nv-follow.following{background:rgba(255,255,255,.86);color:#111}.nv-caption{margin:0;font-size:14px;line-height:1.5;max-width:100%;text-shadow:0 2px 12px rgba(0,0,0,.7);white-space:pre-wrap}.nv-tags{display:flex;gap:7px;flex-wrap:wrap;margin-top:8px}.nv-tags span{font-size:12px;font-weight:700}.nv-sound{display:flex;gap:7px;align-items:center;margin-top:10px;font-size:11px;opacity:.78;max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.nv-sound span:last-child{overflow:hidden;text-overflow:ellipsis}#necpa-status-root .nv-actions{position:absolute;z-index:10;right:max(18px,calc(50% - 315px));bottom:25px;display:flex;flex-direction:column;align-items:center;gap:13px}.nv-action{width:58px;min-height:58px;border:0;background:transparent;color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;cursor:pointer;text-shadow:0 2px 8px rgba(0,0,0,.7);transition:transform .16s ease,filter .16s ease}.nv-action:hover{filter:brightness(1.25)}.nv-action:active{transform:scale(.86)}.nv-action b{width:46px;height:46px;border-radius:50%;display:grid;place-items:center;background:rgba(0,0,0,.42);border:1px solid rgba(255,255,255,.12);backdrop-filter:blur(12px);font-size:22px;font-weight:500}.nv-action span{font-size:10px;font-weight:800;max-width:70px;white-space:nowrap}.nv-action.liked b{background:rgba(220,38,38,.75);animation:nvLike .42s cubic-bezier(.34,1.56,.64,1)}.nv-action.active b{background:rgba(255,255,255,.2)}.nv-avatar-action b{padding:0;overflow:hidden}.nv-action-avatar{width:100%!important;height:100%!important;border-radius:50%;background-size:cover;background-position:center}.nv-count{position:absolute;right:18px;top:104px;z-index:8;font-size:10px;opacity:.6}.nv-count strong{opacity:1;color:#fff}#necpa-status-root .nv-tap-play{position:absolute;z-index:6;left:50%;top:50%;transform:translate(-50%,-50%);width:74px;height:74px;border:0;border-radius:50%;background:rgba(0,0,0,.34);color:#fff;opacity:0;cursor:pointer;backdrop-filter:blur(10px)}.nv-tap-play:after{content:"▶";font-size:25px;margin-left:4px}.nv-stage:hover .nv-tap-play{opacity:.7}#necpa-status-root .nv-loading,#necpa-status-root .nv-empty{position:absolute;inset:0;z-index:25;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:30px;background:var(--kyn-bg-root,#08090d);gap:9px}.nv-loading strong,.nv-empty h2{font-size:20px;margin:5px 0}.nv-loading span,.nv-empty p{max-width:390px;font-size:13px;opacity:.6;line-height:1.5}.nv-empty-mark{width:58px;height:58px;border-radius:20px;display:grid;place-items:center;background:rgba(255,255,255,.07);font-size:28px}.nv-primary{border:0;border-radius:999px;background:var(--status-accent,#7c3aed);color:#fff;padding:11px 18px;font-weight:800;cursor:pointer}.nv-spinner{width:30px;height:30px;border:3px solid rgba(255,255,255,.18);border-top-color:var(--status-accent,#7c3aed);border-radius:50%;animation:nvSpin .8s linear infinite}#necpa-status-root .nv-media-loading{position:absolute;z-index:6;left:50%;top:50%;transform:translate(-50%,-50%);display:flex;flex-direction:column;gap:8px;align-items:center;font-size:11px;text-shadow:0 2px 8px #000}.nv-media-error{position:absolute;z-index:12;left:50%;top:50%;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;gap:12px;padding:20px;border-radius:20px;background:rgba(0,0,0,.72);backdrop-filter:blur(18px);text-align:center}.nv-media-error button{border:0;border-radius:999px;padding:9px 14px;font-weight:800;cursor:pointer}#necpa-status-root .nv-comments{position:absolute;inset:0;z-index:50;background:rgba(0,0,0,.38);display:none;align-items:flex-end;justify-content:center;backdrop-filter:blur(3px)}.nv-comments.open{display:flex}.nv-comments-card{width:min(620px,100%);max-height:76dvh;display:flex;flex-direction:column;background:var(--kyn-bg-root,#101116);border:1px solid rgba(255,255,255,.12);border-radius:24px 24px 0 0;overflow:hidden;box-shadow:0 -20px 70px rgba(0,0,0,.4)}.nv-comments-card header{display:flex;justify-content:space-between;align-items:center;padding:16px 18px;border-bottom:1px solid rgba(255,255,255,.1)}.nv-comments-card header button{border:0;background:rgba(255,255,255,.08);color:#fff;border-radius:50%;width:34px;height:34px;cursor:pointer}.nv-comment-list{overflow:auto;padding:8px 18px;min-height:130px}.nv-comment-list article{padding:12px 0;border-bottom:1px solid rgba(255,255,255,.07);display:flex;flex-direction:column;gap:4px}.nv-comment-list article b{font-size:12px}.nv-comment-list article span{font-size:13px;line-height:1.45;word-break:break-word}.nv-no-comments{text-align:center;opacity:.55;padding:35px}.nv-comment-compose{display:flex;gap:8px;padding:12px 14px;padding-bottom:calc(12px + env(safe-area-inset-bottom,0px));border-top:1px solid rgba(255,255,255,.1)}.nv-comment-compose input{flex:1;min-width:0;border:0;border-radius:999px;background:rgba(255,255,255,.08);color:#fff;padding:12px 15px;outline:none;font-size:16px}.nv-comment-compose button{border:0;border-radius:999px;background:var(--status-accent,#7c3aed);color:#fff;padding:0 17px;font-weight:800;cursor:pointer}#necpa-status-root .nv-search-panel{position:absolute;z-index:45;top:82px;left:50%;transform:translateX(-50%);width:min(650px,calc(100% - 30px));padding:10px;border:1px solid rgba(255,255,255,.12);border-radius:18px;background:rgba(10,11,15,.86);backdrop-filter:blur(24px);box-shadow:0 20px 70px rgba(0,0,0,.45)}.nv-search-panel form{display:flex;gap:8px}.nv-search-panel input{flex:1;min-width:0;border:0;border-radius:12px;background:rgba(255,255,255,.08);color:#fff;padding:12px 14px;outline:none;font-size:15px}.nv-search-panel button{border:0;border-radius:12px;padding:0 15px;background:var(--status-accent,#7c3aed);color:#fff;font-weight:800;cursor:pointer}.nv-search-panel .nv-search-close{background:rgba(255,255,255,.08);padding:0 12px}.nv-more{position:absolute;z-index:55;right:max(18px,calc(50% - 315px));bottom:26px}.nv-more-card{display:flex;flex-direction:column;min-width:180px;padding:6px;border-radius:18px;background:rgba(15,16,22,.94);border:1px solid rgba(255,255,255,.12);backdrop-filter:blur(20px);box-shadow:0 18px 60px rgba(0,0,0,.5)}.nv-more-card button{border:0;background:none;color:#fff;text-align:left;padding:12px 13px;border-radius:12px;cursor:pointer;font-weight:700}.nv-more-card button:hover{background:rgba(255,255,255,.08)}@keyframes nvSpin{to{transform:rotate(360deg)}}@keyframes nvLike{0%{transform:scale(.8)}55%{transform:scale(1.18)}100%{transform:scale(1)}}@media(max-width:800px){#necpa-status-root .nv-top{height:66px;grid-template-columns:auto minmax(0,1fr) auto;gap:10px;padding-left:12px;padding-right:12px}.nv-brand{display:none!important}.nv-tabs{justify-content:flex-start;gap:16px}.nv-tabs button{font-size:12px}.nv-video{width:100%!important;height:100%!important;object-fit:cover!important}.nv-progress{top:70px!important;left:12px!important;right:12px!important}.nv-copy{left:14px!important;bottom:18px!important;width:calc(100% - 92px)!important}.nv-actions{right:8px!important;bottom:18px!important;gap:8px!important}.nv-action{width:58px!important;min-height:50px!important}.nv-action b{width:42px;height:42px;font-size:20px}.nv-action span{font-size:9px}.nv-count{top:84px;right:12px}.nv-search-panel{top:72px!important}.nv-more{right:10px!important;bottom:18px!important}.nv-comments-card{max-height:78dvh}.nv-gradient-bottom{height:48%}}@media(prefers-reduced-motion:reduce){.nv-spinner{animation:none}.nv-action.liked b{animation:none}}';document.head.appendChild(s)})();
...
window.moveVibe=moveVibe;window.toggleVibeLove=toggleVibeLove;window.openVibes=openVibes;
