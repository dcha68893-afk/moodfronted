...
async function reply(s,text){if(!text.trim())return;try{await api('/'+s.id+'/reply',{method:'POST',body:JSON.stringify({text:text.trim()})});s.replyCount=(s.replyCount||0)+1;document.querySelector('[data-reply]').value='';toast('Reply sent privately to the creator')}catch(e){toast(e.message)}}
