...
// VERSION: 1.5.0 - Persistent two-account switching + account-scoped local state preservation
(function () {
    'use strict';
    ...
    // User-specific localStorage/sessionStorage is not automatically isolated by
    // the browser. Save the old account's non-auth application state before a
    // switch, then restore the target account's state after its credentials are
    // activated. This protects legacy modules that still use their own storage
    // keys instead of AppCache, while preserving the two-account limit.
    const GLOBAL_STORAGE_KEYS = new Set([
        'nexopa_theme', 'nexopa_nav_state', ACCOUNT_LIST_KEY, ACCOUNT_STATE_KEY
    ]);
    ...
    function captureAccountState(accountId) {
        ...
        try {
            Object.keys(localStorage).forEach(key => {
                if (GLOBAL_STORAGE_KEYS.has(key) || AUTH_KEYS.has(key)) return;
                try { state.localStorage[key] = localStorage.getItem(key); } catch (_) {}
            });
        } catch (_) {}
        try {
            Object.keys(sessionStorage).forEach(key => {
                if (AUTH_KEYS.has(key)) return;
                try { state.sessionStorage[key] = sessionStorage.getItem(key); } catch (_) {}
            });
        } catch (_) {}
        try {
            const all = getAccountStateMap();
            all[id] = { ...state, savedAt: Date.now() };
            withAuthMutation(() => localStorage.setItem(ACCOUNT_STATE_KEY, JSON.stringify(all)));
        } catch (_) {}
    }
    ...
    // AppDB is account-scoped by app.cache.unified.js and must survive account
    // transitions so Account A's cached data can be restored when returning A.
    ...
    function wipePreviousAccountData() {
        ...
        if (previousUserId != null) captureAccountState(previousUserId);
        try {
    ...
    function removeSavedAccount(userId) {
        ...
        try {
            withAuthMutation(() => {
                localStorage.setItem(ACCOUNT_LIST_KEY, JSON.stringify(next.slice(0, MAX_ACCOUNTS)));
                const allState = getAccountStateMap();
                delete allState[targetId];
                localStorage.setItem(ACCOUNT_STATE_KEY, JSON.stringify(allState));
            });
            try {
    ...
    function saveAuth(data) {
        try {
            ...
            const payload = { token:data.token, refreshToken:data.refreshToken || null, user:data.user || null, expiresAt, issuedAt:data.issuedAt || Date.now(), savedAt:new Date().toISOString(), _version:'1.5.0' };
            withAuthMutation(() => {
    ...
    function switchAccount(userId) {
        ...
        try {
            ...
            const payload = { token:target.token, refreshToken:target.refreshToken || null, user, expiresAt:Object.prototype.hasOwnProperty.call(target,'expiresAt') ? target.expiresAt : null, issuedAt:Date.now(), savedAt:new Date().toISOString(), _version:'1.5.0' };
            withAuthMutation(() => {
                localStorage.setItem(AUTH_STORAGE_KEY,JSON.stringify(payload));
            ...
            // Restore only the target account's application state. Auth keys are
            // intentionally excluded so the target credentials remain canonical.
            ...
            try {
                ...
                if (typeof window.location?.reload === 'function') setTimeout(() => window.location.reload(), 0);
            } catch (_) {}
