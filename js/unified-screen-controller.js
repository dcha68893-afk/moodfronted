/* Necpa Unified Screen + Theme Authority v5
 * chat.html is the single shell. Modules remain isolated documents, but the
 * shell is the only owner of visibility, theme state, and theme transitions.
 */
(function (global, document) {
  ...
  var state=global.__NECPRA_SCREEN_STATE__||{module:'messages',panel:null,panels:{},switching:false,ready:false,theme:null,transitionId:0};
  global.__NECPRA_SCREEN_STATE__=state;
  ...
  /* app_theme is the shell's canonical runtime value. Settings caches are
   * mirrors/fallbacks only. This prevents a stale per-user light cache from
   * overwriting a deliberately saved dark theme during reload/relogin. */
  function readSettingsTheme(){try{var raw=localStorage.getItem('app_theme');if(validTheme(raw))return raw;var keys=['knecta_settings_cache','app_settings_global','necpa_settings_default'];for(var i=0;i<keys.length;i++){raw=localStorage.getItem(keys[i]);if(!raw)continue;var p=JSON.parse(raw),d=p&&(p.data||p),t=d&&(d.appearance&&d.appearance.theme||d.theme);if(validTheme(t))return t;}}catch(_){}return null;}
  function authoritativeTheme(){var saved=readSettingsTheme();if(saved)return saved;try{var t=global.ThemeManager&&global.ThemeManager.getTheme&&global.ThemeManager.getTheme();if(validTheme(t))return t;}catch(_){}var root=document.documentElement.getAttribute('data-theme');if(validTheme(root))return root;return 'light';}
  ...
  function notifyFrameTheme(frame,reason){try{if(frame&&frame.contentWindow)frame.contentWindow.postMessage({type:'NECPRA_THEME_APPLIED',theme:state.theme,fontSize:global.ThemeManager&&global.ThemeManager.getFontSize?global.ThemeManager.getFontSize():16,iconScale:global.ThemeManager&&global.ThemeManager.getIconScale?global.ThemeManager.getIconScale():'medium',reason:reason||'shell'},location.origin);}catch(_) {}}

  function themeFrame(frame,reason){if(!frame||!frame.contentDocument)return;try{var theme=state.theme||authoritativeTheme(),doc=frame.contentDocument;doc.documentElement.setAttribute('data-parent-shell','true');doc.documentElement.setAttribute('data-parent-module',frame.dataset&&frame.dataset.module||'');doc.documentElement.setAttribute('data-theme',theme);doc.documentElement.classList.toggle('theme-dark',theme==='dark');doc.documentElement.classList.toggle('dark-theme',theme==='dark');doc.documentElement.style.colorScheme=theme;copyThemeVars(document,doc);if(doc.body){doc.body.setAttribute('data-theme',theme);doc.body.classList.toggle('dark-theme',theme==='dark');doc.body.setAttribute('data-parent-shell','true');}notifyFrameTheme(frame,reason);frame.dataset.kynThemeReady='true';frame.style.visibility='visible';}catch(_){}
  }
  function themeAll(reason){frames().forEach(function(f){if(f.contentDocument)themeFrame(f,reason);});}

  function applyTheme(theme,source){if(!validTheme(theme))theme=authoritativeTheme();if(theme===state.theme&&document.documentElement.getAttribute('data-theme')===theme){themeAll(source||'same-theme');return false;}state.theme=theme;document.documentElement.setAttribute('data-theme',theme);document.documentElement.classList.toggle('theme-dark',theme==='dark');document.documentElement.classList.toggle('dark-theme',theme==='dark');document.documentElement.style.colorScheme=theme;try{if(global.ThemeManager&&global.ThemeManager.getTheme&&global.ThemeManager.getTheme()!==theme)global.ThemeManager.setTheme(theme,{force:true});}catch(_){}try{localStorage.setItem('app_theme',theme);}catch(_){}themeAll(source||'shell');emit('kyn:themechange',{theme:theme,source:source||'unified-screen-controller'});return true;}

  function refreshTheme(source){var t=authoritativeTheme();if(t===state.theme){if(document.documentElement.getAttribute('data-theme')!==t)applyTheme(t,source);else themeAll(source);return;}var id=state.transitionId+1;switchStart();applyTheme(t,source);switchEnd(id);}
  ...
  function syncModule(module,opts){module=normalize(module);if(!MODULES[module])return;opts=opts||{};var prev=state.module,changed=prev!==module;if(!changed&&!opts.force){shellState();return;}var id=state.transitionId+1;switchStart();state.module=module;if(!opts.keepPanel)state.panel=null;shellState();reveal(module);emit('kyn:screenchange',{module:module,previous:prev,panel:state.panel,source:'unified-screen-controller'});switchEnd(id);}
  ...
  function installPanels(){global.addEventListener('message',function(e){var d=e&&e.data;if(!d||typeof d!=='object')return;if(d.type==='NECPRA_THEME_REQUEST'){if(validTheme(d.theme))applyTheme(d.theme,'settings-request');return;}if(d.type==='NECPRA_FONT_REQUEST'){try{if(global.ThemeManager&&global.ThemeManager.setFontSize)global.ThemeManager.setFontSize(d.value);}catch(_){}return;}if(d.type==='NECPRA_ICON_REQUEST'){try{if(global.ThemeManager&&global.ThemeManager.setIconScale)global.ThemeManager.setIconScale(d.value);}catch(_){}return;}if(d.type==='NECPRA_ACCENT_REQUEST'){try{if(global.ThemeManager&&global.ThemeManager.setAccentColor)global.ThemeManager.setAccentColor(d.value);}catch(_){}return;}if(d.type==='PanelOpened')syncPanel(d.module,d.panel,true);else if(d.type==='PanelClosed')syncPanel(d.module,d.panel,false);else if(d.type==='GROUP_PANEL_OPEN'||d.type==='GROUP_CHAT_OPENED'||d.type==='GROUP_DETAIL_OPENED')syncPanel('group','conversation',true);else if(d.type==='GROUP_PANEL_CLOSED'||d.type==='GROUP_LIST_SHOWN'||d.type==='GO_BACK_TO_LIST')syncPanel('group',null,false);else if(d.type==='CHAT_OPENED'||d.type==='CONVERSATION_OPENED'||d.type==='CHAT_HEADER_UPDATE')syncPanel('messages','conversation',true);else if(d.type==='CHAT_LIST_SHOWN'||d.type==='CHAT_CLOSED'||d.type==='GO_BACK_TO_CHAT_LIST')syncPanel('messages',null,false);});document.addEventListener('kyn:panelstate',function(e){var d=e.detail||{};syncPanel(d.module,d.panel,d.type==='PanelOpened');});}

  function installTheme(){var initial=setAuthoritativeTheme();state.theme=initial;applyTheme(initial,'shell-authority-boot');document.addEventListener('kyn:themechange',function(e){var d=e.detail||{},t=readSettingsTheme()||d.theme;if(validTheme(t)&&t!==state.theme)refreshTheme('theme-event');});global.addEventListener('storage',function(e){if(e.key==='app_theme'||e.key==='knecta_settings_cache'||e.key==='app_settings_global'||e.key==='necpa_settings_default')refreshTheme('storage');});}

  function installFrames(){
    frames().forEach(function(f){
      f.style.visibility='hidden';
      f.addEventListener('load',function(){
        /* The iframe document is never allowed to decide when it is visible.
         * It becomes visible only after the shell has painted the canonical
         * theme into that document. */
        themeFrame(f,'frame-load');
      },false);
      if(f.contentDocument)themeFrame(f,'frame-boot');
      try{
        var doc=f.contentDocument;
        if(doc){
          doc.addEventListener('kyn:themechange',function(){themeFrame(f,'child-theme-event');},false);
          doc.addEventListener('appSettingsChanged',function(event){
            var detail=event&&event.detail||{};
            var settings=detail.settings||detail;
            var t=settings&&settings.appearance&&settings.appearance.theme;
            if(validTheme(t))applyTheme(t,'settings-change');
          },false);
        }
      }catch(_){}
    });
