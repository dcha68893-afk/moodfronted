...
(function () {
  ...
  function markBootCompleted(reason = 'unknown') {
    ...
    try {
      window.dispatchEvent(new CustomEvent('necpa-bootstrap-complete', {
  ...
  window.addEventListener('offline', function() {
    ...
    setReady: function (component) {
      ...
      window.dispatchEvent(
        new CustomEvent(`necpa-${component}-ready`, {
  ...
  const BOOTSTRAP_STATE_MACHINE = {
    ...
    emitStateChange: function (oldState, newState) {
      try {
        const event = new CustomEvent("necpa-bootstrap-state-change", {
          detail: {
            oldState: oldState,
            newState: newState,
  ...
  function userLoggedIn() {
    try {
      ...
      const legacyKeys = ["authToken", "accessToken", "token", "necpa_token", "USER_TOKEN", "kynecta_token", "necpa_jwt_token"];
  ...
  const AUTHENTICATION_GATE = {
    ...
    redirectToLogin: function (reason = "Authentication required") {
      ...
      if (!isLoginPage) {
        try {
          sessionStorage.setItem("necpa_return_path", currentPath);
        } catch (error) {
  ...
  const NavigationController = {
    ...
    lock: function (reason = "bootstrap") {
      ...
      window.dispatchEvent(
        new CustomEvent("necpa-navigation-lock", {
    ...
    unlock: function (reason = "bootstrap_complete") {
      ...
      window.dispatchEvent(
        new CustomEvent("necpa-navigation-unlock", {
          detail: { reason, timestamp: Date.now() },
        })
      );
    },
  ...
  const HARDENED_BOOTSTRAP_CONTROLLER = {
    ...
    registerModulesDeterministic: async function () {
      ...
      window.dispatchEvent(
        new CustomEvent("necpa-modules-registered", {
          detail: {
  ...
  const APP_BOOTSTRAP = {
    ...
    initializeTheme: function () {
      ...
      try {
        ...
        // js/app.core.js — a private 'necpa_theme' key, dark-by-default,
    ...
    initializeNavigationContainer: function () {
      return new Promise((resolve) => {
        ...
        window.dispatchEvent(
          new CustomEvent("necpa-navigation-ready", {
            detail: {
              timestamp: new Date().toISOString(),
              isDegraded: BOOTSTRAP_STATE.isDegraded(),
            },
    ...
    loadIframePage: function (pageConfig) {
      ...
      iframe.addEventListener("load", () => {
        ...
        window.dispatchEvent(
          new CustomEvent("necpa-page-loaded", {
            detail: {
