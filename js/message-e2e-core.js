...
(function (global) {
  ...
  function parseEnvelope(content) {
    ...
    try { const o = JSON.parse(content); return o && o.v === 2 && o.iv && o.ct && o.spk ? o : null; } catch (_) { return null; }
  }
  ...
  function bootstrapSecret() {
    const id = me();
    if (!id) throw new Error('User identity is unavailable');
    const key = `kyn_dm_identity_bootstrap_v2_${id}`;
    let value = null;
  ...
  async function ensureIdentity() {
    ...
    if (!identityReady) {
      identityReady = (async () => {
        const identity = I();
        if (!identity) throw new Error('E2E identity layer unavailable');
  ...
  function notifyResolved(id, plaintext, entry) {
    decryptCache.set(id, plaintext); pending.delete(id); failed.delete(id);
  ...
  async function decryptMessageForDisplay(message, chatId, currentUserId, opts = {}) {
    ...
    catch (_) { return opts.fallbackText === undefined ? '' : opts.fallbackText; }
  }

  async function retryDecrypt(chatId, message) {
    const id = messageId(message); decryptCache.delete(id); failed.delete(id); pending.delete(id);
  ...
  async function cacheRecipientKey(userId, keyEntry) {
    ...
    try { localStorage.setItem('kyn_e2e_bootstrap_pub_' + String(userId), JSON.stringify({ pub: keyEntry.publicKey, keyId: keyEntry.keyId })); return true; } catch (_) { return false; }
  }
  ...
  async function registerPendingDecrypt(messageIdValue, attemptFn, onResolved) {
    ...
    const entry = { subscribers: new Set(onResolved ? [onResolved] : []) }; pending.set(id, entry);
  ...
  async function encryptAttachment(arrayBuffer, chatId, recipientUserId) {
    const identity = await ensureIdentity(); const peer = await identity.publicKeyFor(recipientUserId);
    const shared = await identity.deriveShared(peer.key); const key = await identity.hkdf(shared, pairContext(recipientUserId) + ':attachment');
    const iv = crypto.getRandomValues(new Uint8Array(12)); const ct = await crypto.subtle.encrypt({ name: 'AES-GCM', iv, tagLength: 128 }, key, arrayBuffer);
  ...
  async function decryptAttachment(env, chatId, senderUserId) {
    const identity = await ensureIdentity(); const peer = await identity.publicKeyFor(senderUserId);
    const shared = await identity.deriveShared(peer.key); const key = await identity.hkdf(shared, pairContext(senderUserId) + ':attachment');
  ...
  global.KynectaMessageE2E = {
    init: (...args) => I()?.init?.(...args), encryptForChat, decryptFromChat, decryptMessageForDisplay, retryDecrypt,
    prefetchRecipientKey, cacheRecipientKey, isMessageQueued, isMessageFailed, peekDecryptedText,
    registerPendingDecrypt, encryptAttachment, decryptAttachment,
    getSafetyNumbers: (...args) => I()?.getSafetyNumbers?.(...args),
    get enabled() { return !!I()?.enabled; }, get publicKey() { return I()?.publicKey || null; }, get keyId() { return I()?.keyId || null; },
    getMyUserId: () => I()?.userId || null, clearKeys: () => I()?.clear?.(),
  };
})(window);
