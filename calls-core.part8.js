...
_videoCallHandler: null,
        ...
        _attachCallControls: function() {
            // FIX-DUPLICATE-BUTTON-HANDLERS: this method used to attach its own
            // addEventListener click handlers directly to acceptCallBtn,
            // declineCallBtn, endCallBtn, muteCallBtn and videoCallBtn — the exact
            // same DOM elements that calls-ui.js ALSO wires (via onclick/addListener,
            // calling into window.callCore.acceptCall/endCall/etc.). Every click was
            // therefore processed twice in parallel by two independent handler
            // chains, each doing its own state transition, socket emit, timer, and
            // screen/nav update. That is why calls intermittently failed to reach
            // the receiver, stalled before the active stage, and why Accept/
            // Decline/Cancel/End/Mute/Video clicks in the calling screens were
            // unreliable. calls-ui.js is the sole intended owner of every call
            // control button (it already renders every call screen and already
            // calls back into this engine via window.callCore), so this method is
            // now a deliberate no-op. Engine methods (acceptCall/rejectCall/
            // endCall/etc.) remain fully intact and are reached exclusively through
            // calls-ui.js's handlers.
        },
