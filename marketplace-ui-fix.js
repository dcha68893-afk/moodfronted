...
function _resolveNecpaApiBase() {
    // FIX-DOTENV: prefer the single .env-driven runtime config gateway
    // (js/config.js reading window.__NEXIPA_RUNTIME_CONFIG__.BACKEND_URL,
    // generated by scripts/build-config.js from .env) before any of the
    // other, older fallbacks below.
    try {
        if (typeof window.__getApiBase === 'function') {
            const fromConfig = window.__getApiBase();
...
(function _fixApiBase() {
    ...
    window.__kynAPI.baseUrl = _resolveNecpaApiBase();
...
function _realOpenProduct(productId) {
    ...
    const base = (window.__kynAPI?.baseUrl || _resolveNecpaApiBase()).replace(/\/api$/, '');
