    ...
    class FriendQueueManager {
        ...
        async _makeRequest(method, endpoint, data) {
            ...
            if (!token && window.AppStorage) {
                token = window.AppStorage.get('token')
                    || window.AppStorage.get('necpa_token')
        ...
        _isAuthReady() {
            ...
            if (window.AppStorage) {
                const t = window.AppStorage.get('token') || window.AppStorage.get('necpa_token');
                if (t) return true;
            }
            if (localStorage.getItem('token') || localStorage.getItem('necpa_token')) return true;
            return false;
        }
